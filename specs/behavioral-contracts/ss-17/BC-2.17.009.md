...
2. For each pair of bytes at `cursor` and `cursor+1`:
   ...
   - If `segment_type & 0xE0 == 0x20` → Class segment: `value = path[cursor+1]`;
     push `CipPathSegment::Class(value)`.
   - If `segment_type & 0xE0 == 0x24` → Instance segment: `value = path[cursor+1]`;
     push `CipPathSegment::Instance(value)`.
   - If `segment_type & 0xE0 == 0x30` → Attribute segment: `value = path[cursor+1]`;
