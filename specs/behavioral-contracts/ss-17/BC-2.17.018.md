...
5. At window expiry: when `timestamp.saturating_sub(flow.malformed_window_start_ts) > 300`
   (strict greater-than; 300-second window expired):
   `flow.malformed_in_window = 0`, `flow.malformed_anomaly_emitted = false`,
   `flow.malformed_window_start_ts = timestamp` (window re-seeds at current timestamp).
   ...
   timestamp (`timestamp < malformed_window_start_ts`) yields 0, NOT > 300, and therefore does
...
| EC-008 | 2 malformed frames at ts=100 (malformed_window_start_ts=100, malformed_in_window=2); then 1 malformed frame at ts=50 (backwards/out-of-order timestamp) | `saturating_sub(50, 100) = 0`; elapsed = 0, NOT > 300 → window is NOT reset; `malformed_in_window = 3`; T0814 Possible/Low emitted; guard set. (RULING-EDGECASE-001 §2.2 + §4.5 EC-X2) |
