...
(SEC-001-S168 defense-in-depth) unreachable for conformant IEC-104 traffic; it fires only
on genuinely malformed or adversarial input.
...
3. [Residual-overflow path only — defensive branch, non-conformant input] After the
   frame-walk loop (BC-2.19.026) drains all complete frames from `carry + delivery`, the
   remaining residual partial-frame bytes exceed `MAX_IEC104_CARRY_BYTES = 255`.

## Postconditions

1. **Walk-first ordering**: the frame-walk loop (BC-2.19.026) completes frame extraction
   before any residual-overflow check runs. No delivery is discarded before complete-frame
   extraction (anti-evasion clause; cites F-172-001 and Ptacek/Newsham 1998).
...
3. [Residual-overflow path — defensive branch] If `residual.len() > 255` after the frame
   walk: (a) the offending direction's carry is cleared; (b) the analyzer resyncs
...
2. **Walk-first ordering**: the frame-walk loop (BC-2.19.026) always completes before any
   residual overflow check is applied to the remaining bytes. There is no aggregate-size
...
| EC-003 | Frame walk produces residual = 256 bytes (adversarial/non-conformant, constructed) — first occurrence in C2S | carry_c2s cleared; ONE T0814 (Anomaly/Possible/Medium) emitted; carry_overflow_reported_c2s set |
...
|--------|----------|-------------|
| VP-045 | carry_c2s and carry_s2c are never mixed; each is independently bounded at 255 bytes after the frame walk; proptest with arbitrary (direction, data) sequences verifies isolation and WALK-FIRST ordering | proptest: `proptest_vp045_direction_isolation`, `proptest_vp045_independent_run_equivalence` |
