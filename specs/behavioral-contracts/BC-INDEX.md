...
| BC-2.01.009 | Accept pcapng Format: Transparent Detection via Magic-Byte Probe | P0 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.4: supersedes BC-2.01.004; greenfield; ADR-009 Decision 5/6; STORY-123; v1.1: PC1 over-promise fixed — empty pcapng valid (>=0 packets); H5-1 FIXED; v1.2: C-4 stale error-code sweep — E-INP-008..E-INP-013 added; v1.3: M-1 traceability path corrected; v1.4: H-2 probe consume(4) removed — peek-only via fill_buf (no cursor advance); H-4 zero-packet/EPB-before-IDB disambiguation rule added; M-4 Decision 19 anchor for zero-packet notice; Decision 20 #seq convention note -->
...
| BC-2.01.012 | Parse pcapng Enhanced Packet Block (EPB): Packet Data and Timestamp | P0 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.4: greenfield; ADR-009 Decision 2/4; STORY-125; v1.1: EPB_FIXED_OVERHEAD_BYTES=28, guard-before-allocate, VP-027; v1.2: boundary I-9 clarification; v1.3: H-5 dead-spec removed (per-SHB reset deferred Decision 16); O-1 PC5 interface_id re-cite corrected; v1.4: C-1 EPB padding-aware bound — padding term added to captured_len guard; unconditional body.len() bound added; M-1 crate-enforces over-claim removed; M-3 PC8 scoped to EC-009 only; EC-008 boundary case moved to HS-104 -->
| BC-2.01.013 | Parse pcapng Simple Packet Block (SPB): Packet Data Without Timestamp | P1 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.4: greenfield; ADR-009 Decision 2; STORY-126; v1.1: SPB overhead 16 bytes, no-panic AC; v1.2: 16-byte bound re-stated, HS-107 authored; v1.3: C-1 SPB three-way min(original_len, snaplen, block_body_available) panic fix propagated to PC1+AC-002+EC-007+Case-B; VP-031 assigned (proptest arithmetic correctness); v1.4: M-1 crate-enforces over-claim removed — wirerust performs body-minimum guard on raw-block path -->
| BC-2.01.014 | Pure-Core 64-bit pcapng Timestamp Normalization to (ts_sec, ts_usecs) | P0 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.4: greenfield; ADR-009 Decision 4; VP-025 Kani target; STORY-125; v1.1: saturating arithmetic, VP-025; v1.2: Kani unwind note, base-10 table note; v1.3: M-4 classic-pcap parity scoped to ts_high==0 (ts_high>0 saturates; classic stores raw u32 secs — not bitshift equivalent); v1.4: M-2 if_tsoffset declared out-of-scope — out-of-scope per ADR Decision 21; timestamp formula unchanged -->
| BC-2.01.015 | Unknown pcapng Block Types Are Silently Skipped via block-total-length | P1 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.5: greenfield; ADR-009 Decision 2; STORY-126; v1.1: skip-arm enumeration; v1.2: forward-progress AC, VP-029; v1.3: E-INP-013 route (Decision 15); v1.4: H-5 per-SHB reset dead-spec note removed (Decision 16 / Decision 7); v1.5: H-1 Decision 20 uniform body-decode-truncation rule applied to skip path; M-5 #seq convention note for block sequence numbering -->
...
| BC-2.01.018 | Multi-IDB Link-Type Agreement Policy: Conflict Returns Error (Fail-Closed) | P0 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.5: greenfield; ADR-009 Decision 3; E-INP-011; STORY-124; v1.1: actionable E-INP-011 message, isolation AC; v1.2: STORY-128 re-attribution, OPB-only zero-packet, VP-030; v1.3: H-5 dead-spec EC-005 corrected — multi-section rejected via E-INP-012 (Decision 7); per-SHB reset deferred (Decision 16); v1.4: P3-002 Related-BCs order fix (pass-3 re-audit gap); v1.5: H-3 VP-030 restated — domain narrowed to whitelisted DataLink values only; non-whitelisted → E-INP-001 (out of VP-030 scope per Decision 17); M-4 Decision 19 zero-packet notice anchor; M-5 #seq convention note --> |
