...
| BC-2.11.028 | --no-collapse Opt-Out Flag Disables Terminal Collapse and Restores One-Line-Per-Finding Rendering; JSON/CSV Unaffected | P0 | [WRITTEN] | issue-#259 greenfield | <!-- v1.1 2026-06-17: fix Related BCs stale cross-ref BC-2.13.001→BC-2.13.004 (consistency audit); v1.2 2026-06-17: adv-pass-1 — PC-3 imperative; arch anchors marked insertion targets (F-259-08); v1.3 2026-06-17: adv-pass-9 — F-PA-03: add EC-010 (--no-collapse absent, default --output terminal → collapse applies, default-on); v1.4 2026-06-17: adv-passes-12-14 — F-PB-01: drop '--no-color/--no-reassemble convention' citation (global flags, not subcommand-scoped); replace with subcommand-scoped boolean precedent (#[arg(long)] mitre/dns on Commands::Analyze, cli.rs:150-152); fix stale arch anchor cli.rs:151-153 no_reassemble → cli.rs:150-152 mitre: bool; v1.5 2026-06-17: issue-#62 F2 BC re-anchor — complete wiring note updated to FindingsRender three-way expression; Preconditions + Postconditions + Invariants 1-2 + EC-001..005 updated for enum; v1.6 2026-06-18: F3 adv-round-4 finding 1 (MEDIUM) — PC3/Inv1/Inv6/Arch-Anchor: `*mitre`/`no_collapse` (main() scope only) → `show_mitre_grouping`/`collapse_findings` (run_analyze in-scope params, lines 107-108); main() call-site resolution at lines 79-80 noted; behavior unchanged -->
| BC-2.11.029 | Collapse is Display-Layer Only; JSON/CSV Reporters Receive Unmodified findings Slice; Non-Repeated Findings Individually Visible in All Outputs | P0 | [WRITTEN] | issue-#259 greenfield | <!-- v1.1 2026-06-17: fix PC-3 misleading N=1≤K=3 reasoning (consistency audit); v1.2 2026-06-17: adv-pass-1 — precise csv.rs:40/csv.rs:76 anchors; terminal.rs:63-75 marked insertion target (F-259-05, F-259-08); v1.3 2026-06-17: issue-#62 F2 BC re-anchor (fix-burst) — Precondition 4 + PC-1 inline qualifier + Architecture Anchors: collapse_findings bool → FindingsRender::FlatCollapsed enum; v1.4 2026-06-18: F2 adv-pass-2 anchor-block correction (F-1) — Architecture Anchors corrected: INSERTION TARGET/STORY-118 → REFACTOR TARGET/STORY-120; stale three-field parenthetical → correct four-field v0.8.0 struct; line ranges aligned to BC-2.11.028 sibling treatment (terminal.rs:91-110, main.rs ~373); v1.5 2026-06-18: F3 adv-round-6 sibling-sweep — Architecture Anchor main.rs:~373 wiring expression corrected: `*mitre`/`!no_collapse` (main() scope) → `show_mitre_grouping`/`collapse_findings` (run_analyze in-scope params); sibling of BC-2.11.028 round-4 fix that was missed in round-4 -->
