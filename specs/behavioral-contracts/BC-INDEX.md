...
| BC-2.01.015 | Unknown pcapng Block Types Are Silently Skipped via block-total-length | P1 | [WRITTEN] | feature-pcapng-F2 | <!-- v1.7: greenfield; ADR-009 Decision 2; STORY-126; v1.1: skip-arm enumeration; v1.2: forward-progress AC, VP-029; v1.3: E-INP-013 route (Decision 15); v1.4: H-5 per-SHB reset dead-spec note removed (Decision 16 / Decision 7); v1.5: H-1 Decision 20 uniform body-decode-truncation rule applied to skip path; M-5 #seq convention note for block sequence numbering; v1.6: H-2 OPB-distinct notice — opb_skipped:u32 sub-counter added as dedicated field incremented per obsolete Packet Block (type 0x00000002) skip; skipped_blocks:u32 remains total-skip counter (all skip-arm block types incl. OPB); PC9 rewritten: counters surfaced via PcapSource fields (reader has no filename; main.rs reads post-Ok and emits notice); AC-006 updated; HS-108 Cases d/e cover OPB-only and mixed-skip scenarios; ADR-009 rev 8; v1.7: F-L4 E-INP-010 BC-ref audit — framing-failure path confirmed (btl<12/misaligned/EOF on skipped block fails at crate-framing level → E-INP-010); F-H1 Decision 20 sweep confirmed clean for skip-path error routing -->
