...
>
> **v1.64 2026-06-20 (F2 pass-7 remediation — pcapng-f2-pass7-remediation-2026-06-20):** Inline version annotations synced to on-disk BC frontmatter for 4 BCs: BC-2.01.009 v1.6→v1.7 (F-3 notice DISPLAY arithmetic — generic segment = skipped_blocks - opb_skipped, emitted only when >0; PC6 3-way notice format normalized: OPB-only / generic-only / mixed; F-1 OPB counter "both" model confirmed; Decision 22 ADR-009 rev 9 ref); BC-2.01.012 v1.6→v1.7 (F-4 Precondition 1 contradiction removed — "interface table is non-empty" deleted; EPB decode precedence postcondition added: body.len() guard → read interface_id → empty-table-E-INP-009 → OOB-E-INP-010 → captured_len/padding); BC-2.01.013 v1.6→v1.7 (F-7 symbol-rename lag — remaining block_body_available → spb_data_available in EC-001/002/003 and Canonical Test Vectors; F-8 btl=14 alignment-violation fixture added to EC-005); BC-2.01.015 v1.7→v1.8 (F-1 OPB counter "both" model — PC9/AC-003/AC-006 confirms OPB skip arm increments BOTH skipped_blocks AND opb_skipped; HS-108 Case D/E counter values corrected: Case D skipped_blocks=3, Case E skipped_blocks=3). HS-107 v1.5→v1.6 (F-5 symbol-rename: block_body_available → spb_data_available throughout). HS-108 v1.3→v1.4 (F-1 Case D/E counter values corrected; F-2 obsolete_packet_blocks → opb_skipped (6×); F-3 display arithmetic explicit: G=skipped_blocks-opb_skipped emitted when G>0). 302 active BCs unchanged. error-taxonomy v3.4 unchanged (next_free E-INP-014). VP-INDEX v2.8 unchanged (total 31). D-159.
>
