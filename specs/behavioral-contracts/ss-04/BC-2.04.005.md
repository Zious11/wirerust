...
|--------|----------|-------------|
| VP-TBD | SYN+ACK sets server ISN and state=Established | unit: process SYN then SYN+ACK; assert flow.state and s2c.isn |
| VP-TBD | SYN+ACK-first (no prior SYN) still reaches Established | unit: process SYN+ACK alone |
