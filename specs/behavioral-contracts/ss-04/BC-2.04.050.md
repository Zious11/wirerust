...
|--------|----------|-------------|
| VP-TBD | All state transitions match the table above | unit: test_flow_state_transitions |
| VP-TBD | on_syn() no-op when not in New state | unit |
| VP-TBD | fin_count saturating_add prevents overflow | unit: boundary test |
