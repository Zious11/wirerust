...
|--------|----------|-------------|
| VP-TBD | summarize() contains exactly the documented key set (no missing, no extra keys) | unit: test_summarize_returns_reassembly_stats |
| VP-TBD | flows_completed == flows_fin + flows_rst in every summary | unit: property test |
| VP-TBD | packets_analyzed == packets_tcp, not packets_processed | unit: inject non-TCP packets, assert |
