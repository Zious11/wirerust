...
|--------|----------|-------------|
| VP-TBD | Alert fires exactly once per direction per flow | unit |
| VP-TBD | Normal-sized segment resets run counter | unit: run_of_5_small + 1_large; assert no alert |
| VP-TBD | Port-exempt flows never alert | unit: port=23; 1000 small segments |
...
- `src/reassembly/mod.rs:457-488` -- small-segment threshold check and emission
- `src/reassembly/mod.rs:356-370` -- small_segment_run counter maintenance in insert_payload_segment
- `src/reassembly/config.rs:62-104` -- small_segment_ignore_ports field

## Source Evidence

| Property | Value |
|----------|-------|
| **Path** | `src/reassembly/mod.rs:457-488` |
| **Confidence** | high |
