...
|--------|----------|-------------|
| VP-TBD | flush_contiguous delivers only contiguous prefix | unit: insert with gap; assert partial flush |
| VP-TBD | base_offset monotonically increases | proptest: random insert order; assert base_offset never decreases |
| VP-TBD | bytes_reassembled equals sum of flushed chunks | unit |
...
- `src/reassembly/mod.rs:517-533` -- flush_contiguous_data: calls flush, dispatches on_data
