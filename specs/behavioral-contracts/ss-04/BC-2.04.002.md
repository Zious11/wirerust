...
|--------|----------|-------------|
| VP-TBD | packets_skipped_non_tcp increments exactly once per non-TCP call | unit: single UDP packet |
| VP-TBD | No flow state created for non-TCP input | unit: assert flows.is_empty() after N non-TCP packets |
