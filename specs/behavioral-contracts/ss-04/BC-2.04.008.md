...
|--------|----------|-------------|
| VP-011 | Out-of-order segments delivered in ISN-relative order after gap fill | unit: insert 2,1; assert on_data order |
| VP-011 | buffered_bytes accounting correct throughout | proptest: random insert order; assert buffered_bytes == sum(segment lengths) |
