...
|--------|----------|-------------|
| VP-TBD | total_memory == sum(flow.memory_used()) after every operation | proptest: random sequence of inserts/flushes/closes |
| VP-TBD | total_memory never exceeds memcap + max segment size (bounded by eviction) | proptest |
