...
modified:
  ...
  - "v1.7: DF-SIBLING-SWEEP-001 HS-043 re-anchor: mod.rs:273-279 → mod.rs:303-308 (RST block in apply_handshake_flags); mod.rs:162 → mod.rs:191 (flush_contiguous_data call in process_packet). All prose, EC, Architecture Module, Architecture Anchors, and Source Evidence updated. — 2026-06-01"
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
per-packet flush at `mod.rs:191` before RST arrives; non-contiguous remainder behind a gap is
...
2. Any remaining contiguous data in both directions is flushed to the handler via `on_data`
   ...
   flush at `src/reassembly/mod.rs:191` (unconditional `flush_contiguous_data` after every
...
| EC-002 | RST on Established flow with buffered data | Contiguous data already delivered pre-RST via per-packet flush at `mod.rs:191`; non-contiguous buffered data silently dropped at close (cannot flush behind gap); on_flow_close(Rst); total_memory released. See PC2 enforcement-mode. |
...
| — | Buffered data delivered before on_flow_close (defense-in-depth invariant) | code review: close_flow flush loop body at `lifecycle.rs:52-59` (mirrors BC-2.04.029 v1.4 PC1/PC2/PC3 + BC-2.04.048 v1.3 PC2 / ADR-0004 amendment enforcement-mode precedent; per-packet flush at mod.rs:191 already drains contiguous prefix pre-close, so this VP cannot be unit-tested through close_flow path) |
...
| Architecture Module | SS-04 (reassembly/mod.rs:303-308, RST block; lifecycle.rs:36-62, close_flow) |
| Stories | STORY-019 |
...
- `src/reassembly/mod.rs:303-308` -- RST block in apply_handshake_flags
...
| Property | Value |
|----------|-------|
| **Path** | `src/reassembly/mod.rs:303-308` |
| **Confidence** | high |
