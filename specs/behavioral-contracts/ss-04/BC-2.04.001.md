...
|--------|----------|-------------|
| VP-TBD | All five invalid configs cause panic | unit: #[should_panic(expected = "...")] for each |
| VP-TBD | Valid config (all > 0) never panics | unit: new with min valid config (all fields = 1) |
