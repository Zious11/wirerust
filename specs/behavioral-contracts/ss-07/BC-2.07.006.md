...
modified:
  ...
  - "v1.5: PG-ARP-F2-007 ss-07 full re-anchor — is_grease_u16 50-52→51-53; cipher filter 100-106→102-107; ext filter 108-121→109-122; curves filter 123-143→124-144; compute_ja3s ext filter 157-169→158-170 — 2026-06-13"
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
| Architecture Module | SS-07 (analyzer/tls.rs:51-53, 102-144 [GREASE-filter sub-region of compute_ja3], C-13) -- cipher filter 102-107, ext filter 109-122, curves filter 124-144. Note: 102-144 is the sub-region containing GREASE filter logic only; sibling BC-2.07.007 anchors the whole compute_ja3 function (doc-block + body, 93-152) for string-format + MD5 behavior. |
| Stories | STORY-051 |
...
- `src/analyzer/tls.rs:51-53` -- `is_grease_u16` function
- `src/analyzer/tls.rs:102-107` -- cipher GREASE filtering in compute_ja3
- `src/analyzer/tls.rs:109-122` -- extension type ID GREASE filtering in compute_ja3
- `src/analyzer/tls.rs:124-144` -- named-group GREASE filtering in compute_ja3 (curves and point-format extraction)
...
| Property | Value |
|----------|-------|
| **Path** | `src/analyzer/tls.rs:51-53` (is_grease_u16), `src/analyzer/tls.rs:102-144` (compute_ja3: cipher filter 102-107, ext filter 109-122, curves/pf filter 124-144), `src/analyzer/tls.rs:158-170` (compute_ja3s ext filter) |
| **Confidence** | high |
...
anchors the GREASE-filtering sub-region of `compute_ja3` (lines 102-144: cipher filter
102-107, extension filter 109-122, curves filter 124-144) and the corresponding
sub-region in `compute_ja3s` (158-170). Sibling BC-2.07.007 anchors the WHOLE
`compute_ja3` function (doc-block + body, lines 93-152) for its string-format + MD5
