...
`captured_len = min(original_len, snaplen_from_idb[0], block_body_available)` where
`block_body_available = block_total_length - 16`, and strip the padding accordingly.
The data slice MUST be bounded by the actual body length unconditionally first so that
a malformed SPB with original_len or snaplen exceeding body.len() cannot produce an
out-of-bounds slice.
...
1. The raw `data` slice from `RawBlock` is the block body after `original_len` (4 bytes),
   ...
   `captured_len = min(original_len, snaplen_from_idb[0], block_body_available)`.
   The data slice MUST be bounded by `block_body_available` unconditionally first (so no
   slice can ever exceed `body.len()`), then further clamped by the three-way minimum.
...
6. **SPB error routing — uniform split (ADR-009 rev 7 Decision 20).**
   ...
   - **btl = 12 (aligned, ≥12; crate frames and returns block)** — body = 0 bytes (12−12=0),
     ...
     Canonical fixture: btl=12 → body=0 bytes < 4 → E-INP-008. (HS-107 btl=12→E-INP-008
     holdout deferred to a separate burst.)
...
- **AC-002 (padding strip):** The raw `data` slice from the crate INCLUDES padding bytes to
  ...
  `captured_len = min(original_len, snaplen, block_body_available)` where
  `block_body_available = block_total_length - 16`, and slice to exactly `captured_len` bytes
  before populating `RawPacket.data`. The slice MUST be bounded by `block_body_available`
  unconditionally first so that a malformed SPB where original_len or snaplen exceeds the
  actual body cannot produce an out-of-bounds slice. Handing the padded or unbounded slice
...
- **AC-004a (Body-too-short — E-INP-008):** An SPB where the crate returns a valid-framed
  ...
  body=0 bytes < 4 → E-INP-008. (HS-107 btl=12→E-INP-008 holdout deferred to a separate burst.)
...
2. Packet data is bounded by the three-way minimum
   `min(original_len, snaplen, block_body_available)` where
   `block_body_available = block_total_length - 16`; no out-of-bounds read is possible.
   The body-available bound is applied unconditionally first.
...
| EC-007 | `original_len` > `snaplen`; block body contains at most `snaplen` captured bytes + padding (the capture device truncated to snaplen; original_len > snaplen bytes on wire were never stored) | `captured_len = min(original_len, snaplen, block_body_available) = snaplen` (snaplen wins because body holds only snaplen bytes); data sliced to `snaplen` bytes |
| EC-008 | SPB with btl=12 (aligned, crate frames and returns block; body=0 bytes < 4 SPB fixed-field bytes for original_len:u32) | `Err` mapping to **E-INP-008** (body-too-short; wirerust body-decode checks body.len()>=4 itself). No panic. Constructible window for SPB body-too-short: btl=12 only (body=0). btl<12 would be E-INP-010. (HS-107 btl=12 holdout deferred to separate burst.) **Test:** `test_BC_2_01_013_spb_body_truncated_e_inp_008` |
...
| VP-031 | For all (original_len: u32, snaplen: u32, body: &[u8]): `captured_len == min(original_len, snaplen, body.len() as u32)` and the returned data slice has EXACTLY `captured_len` bytes with no out-of-bounds access | proptest: generate arbitrary (original_len, snaplen, body) triples; assert framing arithmetic and slice length; Phase P1 |
| HS-107 | SPB holdout scenario: real-world pcapng file with SPB blocks validates end-to-end ingestion correctness and no false positives/negatives on SPB-carrying captures | holdout evaluation (Phase 4); see `.factory/holdout-scenarios/HS-107-pcapng-spb-framing-truncation-padding-and-no-idb.md` |
