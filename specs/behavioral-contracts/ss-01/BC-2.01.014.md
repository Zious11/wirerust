...
modified:
  ...
  - "v1.6: Spec-accuracy fix — corrected arithmetically-impossible EC-013 saturation-guard canonical test vector (ADR-009 rev 8 follow-up). Prior vector used ts_high=4295, claiming ticks=4295*2^32=18_448_744_073_709_551_616 (exceeds u64::MAX=18_446_744_073_709_551_615 — physically impossible) and ticks/1_000_000=18_448_744_073_709 (also wrong). True value: 4295*2^32=18_446_884_536_320, which divides to 18_446_884 — far below u32::MAX, meaning ts_high=4295 does NOT saturate at µs resolution. Replaced with ts_high=2_000_000, ts_low=0: ticks=2_000_000*2^32=8_589_934_592_000_000; ticks/1_000_000=8_589_934_592 which exceeds u32::MAX(4_294_967_295) → ts_sec=u32::MAX (saturated), ts_usecs=0. Saturation threshold: any ts_high >= 1_000_000 (with ts_low=0) saturates at µs resolution; 2_000_000 is a clean value above it. Normative behavior unchanged — only the wrong example numbers are corrected. VP-025 Kani saturation assertion updated to use the corrected ts_high=2_000_000 vector. — 2026-06-20"
...
| EC-013 | `ts_high=2_000_000, ts_low=0, if_tsresol=6` (µs fast path, ts_high large enough that ticks/1_000_000 > u32::MAX) | `ts_sec=u32::MAX` (saturated via `.min(u32::MAX as u64)`); `ts_usecs=0`; NO PANIC. Arithmetic: `ticks = 2_000_000 << 32 = 8_589_934_592_000_000`; `ticks / 1_000_000 = 8_589_934_592` which exceeds `u32::MAX = 4_294_967_295` → saturation triggers. Saturation threshold: any `ts_high >= 1_000_000` (with ts_low=0) saturates at µs resolution (exact: `ts_high=1_000_000` → `ticks/1_000_000=4_294_967_296 > u32::MAX`); 2_000_000 is a clean value above it. This is the FAST-PATH SATURATION test (M-3): without `.min()`, the bare `as u32` cast wraps and produces a value < u32::MAX, diverging from the general formula. VP-025 Kani harness MUST include this assertion. |
...
| `ts_high=2_000_000, ts_low=0, if_tsresol=6` | `ts_sec=u32::MAX` (saturated; `ticks=2_000_000*2^32=8_589_934_592_000_000`; `ticks/1_000_000=8_589_934_592` which exceeds `u32::MAX=4_294_967_295`; fast path MUST saturate via `.min(u32::MAX as u64)`); `ts_usecs=0`; NO PANIC — fast-path saturation guard (M-3 / VP-025) | saturation (M-3) |

## Verification Properties

| VP-NNN | Property | Proof Method |
|--------|----------|-------------|
| VP-025 | `pcapng_timestamp_to_secs_usecs` totality: no panic for ALL (u32, u32, u8) inputs with saturating arithmetic; `ts_usecs` always in [0, 999_999]; `ts_sec` plausible (≤ u32::MAX); denominator never 0 | Kani: `#[kani::proof]` over full symbolic input space (u32 × u32 × u8 = 2^65 inputs; Kani explores exhaustively via bounded model checking). **Implementation note (I-2):** the base-10 branch MUST use a precomputed ticks_per_sec lookup table for e∈[0,19] (saturating to u64::MAX for e≥20) — **Option A (preferred)**: keeps the Kani proof bounded without unwind annotations; OR the VP-025 Kani harness carries `#[kani::unwind(128)]` — **Option B**. Without one of these, the Kani proof over the base-10 `checked_pow` loop is vacuous (Kani will not explore all paths). Option A is preferred. **Fast-path saturation (M-3):** the VP-025 Kani harness MUST NOT short-circuit the `if_tsresol == 6` branch — this branch now includes a `.min(u32::MAX as u64) as u32` saturation that the Kani proof must verify holds (i.e., `ts_sec ≤ u32::MAX` even when `ticks / 1_000_000 > u32::MAX`, e.g., `ts_high ≥ 1_000_000` with ts_low=0). The saturation test vector `(ts_high=2_000_000, ts_low=0, if_tsresol=6) → ts_sec=u32::MAX` must be included as a concrete assertion in the Kani harness. Arithmetic: ticks=2_000_000*2^32=8_589_934_592_000_000; ticks/1_000_000=8_589_934_592 > u32::MAX=4_294_967_295 → saturates. (Saturation threshold: any ts_high >= 1_000_000 with ts_low=0 saturates at µs resolution; exact minimum: 1_000_000*2^32/1_000_000=2^32=4_294_967_296 > u32::MAX.) |
