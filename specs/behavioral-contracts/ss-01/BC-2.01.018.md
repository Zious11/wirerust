...
modified:
  - "v1.6: Pass-5 remediation S4 (ADR-009 rev 8 Decision 17 precedence) — EC-006 CORRECTED: ETHERNET (whitelisted) then IEEE802_11 (non-whitelisted) → E-INP-001 fires on the SECOND IDB at whitelist check (#2); E-INP-011 conflict check (#3) is NEVER reached because whitelist preempts conflict. Old text incorrectly stated E-INP-011 fires first. EC-008 RE-DERIVED: two IEEE802_11 IDBs — the FIRST IDB (non-whitelisted) already triggers E-INP-001 at whitelist check (#2) during first-IDB parse time; the second IDB is NEVER parsed; the agreement/disagreement between the two IDBs is unobservable. Old text described a defunct order where agreement was checked before whitelist. E-INP-011 conflict check is reachable ONLY when BOTH IDBs are whitelisted AND differ (e.g. ETHERNET then RAW). — 2026-06-20"
...
| EC-006 | Two IDBs: `ETHERNET` (whitelisted) then `IEEE802_11` (non-whitelisted) | **E-INP-001** fires on the SECOND IDB at whitelist check (#2 per Decision 17); E-INP-011 conflict check (#3) is NEVER reached because whitelist preempts conflict. The first IDB (ETHERNET) passes the whitelist. The second IDB (IEEE802_11) fails the whitelist before any conflict comparison occurs. Error: E-INP-001 citing IEEE802_11 as non-whitelisted. |
...
| EC-008 | Two IDBs both `IEEE802_11` (non-whitelisted but agreeing) | **E-INP-001** fires on the FIRST IDB at whitelist check (#2 per Decision 17) during first-IDB parse time; the second IDB is NEVER parsed and the agreement between the two IDBs is completely unobservable. The defunct narrative ("they agree so E-INP-011 doesn't fire") described a non-existent order where agreement was checked before whitelist. Correct behavior: IEEE802_11 hits whitelist check (#2) → E-INP-001 immediately; the reader stops before any conflict comparison. |
