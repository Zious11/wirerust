...
6. Packet data slice validation uses a two-step, padding-aware check (applied in this order,
   ...
   a. **Bound-by-body first (unconditional):** `captured_len <= body.len()`. A
      ...
      return `Err` mapping to E-INP-010 regardless of what `block_total_length` says.
   b. **Padding-aware overhead check:** `EPB_FIXED_OVERHEAD_BYTES(20) + captured_len +
      ...
      to E-INP-010 (block-length inconsistency / padding overrun).
...
- **AC-002 (guard-before-allocate, SEC-004):** Two validations MUST be performed BEFORE any
  ...
  1. Unconditional bound-by-body: `captured_len <= body.len()` — the data slice can NEVER
     exceed the raw body length regardless of `block_total_length`.
...
- **AC-006 (maximum-boundary captured_len fidelity, EC-009):** When `captured_len` equals
  ...
  causes the padded total to exceed `body.len()` MUST return `Err` mapping to E-INP-010.
...
| EC-010 | `captured_len` one byte over the padding-aware boundary: `20 + captured_len + pad_len(captured_len) > body.len()` | `Err` mapping to E-INP-010 (padded total exceeds body — block-length inconsistency / padding overrun) |
...
| EPB where `20 + captured_len + pad_len(captured_len) > body.len()` (one byte over padding-aware boundary) | `Err` mapping to E-INP-010 (padding overrun) | boundary-invalid (EC-010) |
...
|--------|----------|-------------|
| VP-027 | EPB parse safety: no panic; interface_id bounds-check before table index (empty-table → E-INP-009, OOB-non-empty → E-INP-010); body.len() >= 20 check (wirerust-owned) before any fixed-field read (body < 20 → E-INP-008); unconditional bound-by-body check (`captured_len <= body.len()`) followed by padding-aware overhead check (`20 + captured_len + pad_len(captured_len) <= body.len()`) both precede any allocation; returns Err for all invalid inputs including padding-overrun case | Kani: `#[kani::proof]` over EPB byte sequences with symbolic interface_id, captured_len, and body length; includes body-length=19 (EC-011) and padded-overrun (EC-010) cases |
...
- ADR-009 rev 4 Decision 4: `EPB_FIXED_OVERHEAD_BYTES = 20` (body-relative); superseded by ADR-009 rev 7 Decision 20 padding-aware validation: (1) unconditional `captured_len <= body.len()`; (2) `20 + captured_len + pad_len(captured_len) <= body.len()` where `pad_len(n) = (4 - n%4) % 4`; body < 20 bytes → E-INP-008; padding-overrun → E-INP-010
- ADR-009 rev 7 Decision 20 (H-1): EPB body-too-short taxonomy — `12 <= btl < 32` → body < 20 → wirerust body-decode failure → E-INP-008 (NOT E-INP-010); `btl < 12` or trailing-length mismatch → crate Err → E-INP-010; EPB fixed-field minimum = 20 body bytes; wirerust owns the `body.len() >= 20` check (not delegated to crate)
