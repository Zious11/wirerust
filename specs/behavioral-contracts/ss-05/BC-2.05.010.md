...
1. **TCP counter — on_flow_close for None-target flows:**
   - Let `lower_port = flow_key.lower_port().min(flow_key.upper_port())` (direction-normalized; approximates the server/service port).
...
- `src/dispatcher.rs` — `on_flow_close` None-target arm: the `unclassified_port_counts` increment is co-located with `unclassified_flows += 1` and is gated on both (a) `self.coverage_gaps_enabled` and (b) the analyzer-present guard (`self.http.is_some() || self.tls.is_some() || self.modbus.is_some() || self.dnp3.is_some() || self.enip.is_some()`); `lower_port = flow_key.lower_port().min(flow_key.upper_port())`; when no analyzers are configured neither `unclassified_flows` nor `unclassified_port_counts` fire (ADR-012 Decision 6 Clarification)
