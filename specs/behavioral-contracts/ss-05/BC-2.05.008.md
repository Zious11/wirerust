...
modified:
  ...
  - v1.6: Pass-18 B-01/B-02/B-03 — re-anchored dispatcher.rs line citations to current post-ICS positions (stale :121-123 → current :256-259); widened Description, Preconditions, Postconditions, Invariants, EC, and Evidence to reflect four-analyzer early-return guard (http/tls/modbus/dnp3) replacing the stale two-analyzer guard (http/tls). — 2026-06-13
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
If `StreamDispatcher` is created with all four analyzers absent (`http = None`, `tls = None`,
`modbus = None`, `dnp3 = None`), then `on_data` returns immediately at the very first check
(dispatcher.rs:256-259) without running `classify`, without updating any counters, and without
touching the `routes` or `classification_attempts` maps. This is the "no-op dispatcher" path
for captures where no analysis is requested.

## Preconditions

1. `StreamDispatcher::new(None, None, None, None)` was called (all four analyzers absent).
2. `on_data` is called for any FlowKey.

## Postconditions

1. Function returns immediately after the `if self.http.is_none() && self.tls.is_none() && self.modbus.is_none() && self.dnp3.is_none()` check.
2. `routes` unchanged.
...
3. A dispatcher configured with at least one analyzer (e.g., http=Some, tls=None, modbus=None,
   dnp3=None) is NOT subject to this early return. ALL FOUR must be None for early-return to
   trigger.
...
|----|-------------|-------------------|
| EC-001 | http=None, tls=None, modbus=None, dnp3=None; any data | on_data returns immediately |
| EC-002 | http=Some, tls=None, modbus=None, dnp3=None; data starts with `0x16 0x03` (TLS content-signature bytes) | Not early-returned (guard requires ALL FOUR None); classify fires and returns DispatchTarget::Tls via content match — port-fallback is NOT reached; tls=None so the Tls dispatch arm is a no-op (no data forwarded); route cached as Tls |
| EC-003 | http=None, tls=None, modbus=None, dnp3=None; on_flow_close | Close still processes normally (no early return in on_flow_close) |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| StreamDispatcher(None, None, None, None).on_data(...) | Returns immediately; routes empty | happy-path |
| StreamDispatcher(Some(http), None, None, None).on_data(TLS bytes) | Not early-returned; classify runs | edge-case |
...
| Architecture Module | SS-05 (dispatcher.rs:256-259, C-21) |
| Stories | STORY-033 |
...
- `src/dispatcher.rs:256-259` -- early-return guard in on_data (four-analyzer: http/tls/modbus/dnp3 all None)
...
| Property | Value |
|----------|-------|
| **Path** | `src/dispatcher.rs:256-259` |
| **Confidence** | high |
| **Extraction Date** | 2026-05-20 |

## Evidence Types Used

- **guard clause**: `if self.http.is_none() && self.tls.is_none() && self.modbus.is_none() && self.dnp3.is_none() { return; }`
- **test-vector**: all-four-None early-return path directly verified by test_BC_2_05_008_no_analyzer_dispatcher_early_returns (unclassified_flows stays 0 after on_flow_close with all four analyzers None); single-analyzer non-early-return verified by test_BC_2_05_008_single_analyzer_not_early_returned (http=Some/tls=None routes GET bytes; http=None/tls=Some routes TLS bytes)
