...
| EC-010 | --no-collapse absent, default --output (terminal) | Collapse applies (default-on); `TerminalReporter.render = FindingsRender { grouping: Grouping::Flat, collapse: Collapse::Collapsed }`; collapsed groups with (xN) suffixes rendered per BC-2.11.025/026 |
...
| Stories | STORY-118, STORY-122 (A — preserves --no-collapse flag wiring and struct construction byte-identical; no CLI change), STORY-119 (B — activates {Grouped,Collapsed} default and {Grouped,Expanded} opt-out; BC-030 governs the grouped-mode CLI mapping introduced by STORY-119) |
...
STORY-118, STORY-122 (A — struct construction preserved; no CLI change), STORY-119 (B — activates grouped default; BC-030 governs grouped CLI mapping)
