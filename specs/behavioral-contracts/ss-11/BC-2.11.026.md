...
4. **OBSERVABLE LINE ORDER — FLAT-COLLAPSE PATH ({Flat, Collapsed}):** For a collapsed group
   ...
   MITRE line using `mitre_techniques.join(", ")` format IF `mitre_techniques` is non-empty
   (BC-2.11.017 PC-6). The ` (xN)` suffix MUST NOT appear on the MITRE line or any evidence
   line — it appears only on the header line. The ` (xN)` suffix is colorized identically
   with the rest of the header line (suffix is part of the pre-colorization string).
