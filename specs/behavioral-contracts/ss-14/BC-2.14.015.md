...
# BC-2.14.015: Write FC to Coil Output Only ({0x05, 0x0F}) Emits Finding Tagged ["T1692.001","T0835"]
...
<!-- Previous version (v1.0): "Write FC to Coil Output Only ({0x05, 0x0F}) Emits T0835 Manipulate I/O Image Finding"
     ...
     v2.0 model (Decision 13): ONE finding per PDU, mitre_techniques: ["T1692.001","T0835"] for coil writes.
...
vec!["T1692.001", "T0835"]`. T0835 ("Manipulate I/O Image") is included because these FCs write
...
authorization. Per Decision 13 (ADR-006), the T0835 attribution and the T1692.001 attribution
...
3. `function_code` is one of: `0x05`, `0x0F`. These are the coil-output write FCs for
   ...
   FC 0x15 carries T1692.001 only (file record write, not register/coil). Precondition 3 is
...
1. ONE `Finding` is pushed (the same per-PDU finding specified in BC-2.14.013 postcondition 1
   for coil-write FCs) with `mitre_techniques: vec!["T1692.001", "T0835"]`:
   ...
   - `mitre_techniques: vec!["T1692.001", "T0835"]`
...
4. **No separate T0835 finding object is created.** The T0835 attribution is carried in
   `mitre_techniques` of the single per-PDU finding. The T1692.001 and T0835 attributions
...
2. **FCs NOT in T0835 subset and why:**
   ...
   - 0x17 Read/Write Multiple Registers: carries T1692.001 + T0836 (holding-register write;
...
|----|-------------|-------------------|
| EC-001 | FC 0x05 writing coil 0x0000 (coil 0, value=ON, 0xFF00) | ONE finding: `mitre_techniques: ["T1692.001","T0835"]`. No T0836 (0x05 is not a holding-register write FC). |
| EC-002 | FC 0x05 writing coil 0x0000 with value 0x0000 (= OFF) | ONE finding: `mitre_techniques: ["T1692.001","T0835"]`. Turning off an actuator is equally significant forensically. |
| EC-003 | FC 0x0F bulk-writing 2000 coils (max spec-compliant bulk coil write) | ONE finding `mitre_techniques: ["T1692.001","T0835"]`; the quantity field is captured in evidence bytes. |
| EC-004 | FC 0x17 (Read/Write Multiple Registers) | ONE finding `mitre_techniques: ["T1692.001", "T0836"]` (holding-register write; in T0836 set {0x06,0x10,0x16,0x17} per BC-DISCREPANCY-001; NOT in coil-write T0835 set). |
| EC-005 | FC 0x16 in request direction | ONE finding `mitre_techniques: ["T1692.001","T0836"]` — mask-write is a holding-register operation, handled by BC-2.14.014. T0835 not applicable (not a coil write). |
| EC-006 | FC 0x15 (Write File Record) | ONE finding `mitre_techniques: ["T1692.001"]` only. T0835 not applicable. |
| EC-007 | `all_findings` at MAX_FINDINGS - 1 when FC 0x05 (coil write) arrives | The ONE finding with `["T1692.001","T0835"]` fills the last slot. `dropped_findings` stays 0. Counters incremented. If cap is already at MAX_FINDINGS, finding is dropped and `dropped_findings += 1`. |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| ADU: `00 01 00 00 00 06 01 05 00 00 FF 00` (FC=0x05 Write Single Coil, coil=0, value=ON, UnitID=1) — ClientToServer | ONE finding: `mitre_techniques=["T1692.001","T0835"]`; `write_count=1`, `fn_code_counts[0x05]=1` | happy-path (coil write, union tag) |
| ADU: `00 02 00 00 00 08 01 0F 00 00 00 04 01 0F` (FC=0x0F Write Multiple Coils, UnitID=1, qty=4, byte=0x0F) — ClientToServer | ONE finding: `mitre_techniques=["T1692.001","T0835"]`; `write_count=1`, `fn_code_counts[0x0F]=1` | happy-path (bulk coil) |
| ADU: `00 03 00 00 00 09 01 17 00 00 00 01 00 00 00 01 02 00 42` (FC=0x17 RW-Multiple, UnitID=1) — ClientToServer | ONE finding: `mitre_techniques=["T1692.001","T0836"]` (FC 0x17 is a holding-register write; NOT a coil-write FC; BC-DISCREPANCY-001 reconciliation) | edge-case (RW-multiple, T0836 tagged, T0835 does NOT apply) |
| ADU: `00 04 00 00 00 06 01 06 00 10 01 F4` (FC=0x06 Write Single Register) — ClientToServer | ONE finding: `mitre_techniques=["T1692.001","T0836"]`; T0835 NOT applicable (holding-register write, not coil write) | negative (register write — T0835 does NOT apply to 0x06) |
| ADU: `00 05 00 00 00 08 01 16 00 10 FF F0 00 0F` (FC=0x16 Mask Write) — ClientToServer | ONE finding: `mitre_techniques=["T1692.001","T0836"]`; T0835 NOT applicable (mask-write is holding-register) | negative (mask-write — T0835 excluded) |
...
| MITRE Techniques | T0835 — Manipulate I/O Image (ATT&CK for ICS; IcsImpairProcessControl tactic); T1692.001 — Unauthorized Message: Command Message (always co-tagged; v19 successor to revoked T0855) |
...
- `src/analyzer/modbus.rs` — I/O image write detection branch in `on_data`; sets `mitre_techniques: vec!["T1692.001","T0835"]` for FC {0x05,0x0F}
...
| Property | Value |
|----------|-------|
| **Path** | f2-fix-directives.md §13.5 (per-PDU emission rules: FC 0x05/0x0F → ["T1692.001","T0835"]); ADR-006 (multi-tag design decision); architecture-delta.md §2.6 (T0835 detection trigger: coil-only I/O image writes {0x05, 0x0F}) |
| **Confidence** | high |
