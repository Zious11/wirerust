...
# BC-2.14.004: MBAP ADU Rejected When Length Is Outside [2, 253] (3-Point Gate: Length Check)
...
253. The Modbus.org spec V1.1b3 defines `Length` as the byte count of Unit ID + PDU: minimum
is 1 (Unit ID) + 1 (FC-only PDU) = 2; maximum is 1 (Unit ID) + 252 (maximum PDU data) = 253.
Values outside [2, 253] indicate either a malformed frame or a non-Modbus protocol on port 502.
...
flags frames exceeding 260 bytes — the 3-point Length gate fires earlier at the 253 PDU limit).
...
2. `h.protocol_id == 0x0000` (the Protocol ID check in `is_valid_modbus_adu` passes — or
   ...
   behavior is observable whenever `h.length < 2` or `h.length > 253`, regardless of the
   Protocol ID check result.
3. `h.length` is outside the valid range: `h.length < 2` OR `h.length > 253`.
...
4. The offset is NOT advanced by `6 + (h.length as usize)`. The `h.length` value is malformed
   (outside [2, 253]) and must not be used in arithmetic — advancing by it could skip past
...
1. **Valid Length range [2, 253]** (Modbus.org spec V1.1b3, §4.1 ADU constraints):
   - Minimum: `Length = 2` = Unit ID (1 byte) + minimum PDU of 1 byte (FC alone).
   - Maximum: `Length = 253` = Unit ID (1 byte) + 252-byte PDU. This equals the RS-485
     constraint harmonized in V1.1b3 (earlier V1 used 249; use 253 per the research).
   - Maximum Modbus TCP ADU byte count: `7 (MBAP header) + 253 = 260 bytes` — confirmed.
...
3. **Length = 254..=65535 are illegal**: the `u16` field can represent up to 65535 but any
   value > 253 is non-spec. The check `h.length <= 253` gates this.
...
6. **No attacker-controlled advance**: the malformed `h.length` value (< 2 or > 253) is never
...
| EC-004 | `length = 253` (maximum valid) | `true` (at the upper bound); ADU is processed normally — NOT rejected by Length gate |
| EC-005 | `length = 254` (first invalid above max) | `false` (254 > 253); `parse_errors++`; no finding |
| EC-006 | `length = 65535` (u16::MAX) | `false` (> 253); `parse_errors++`; loop `break`s; segment discarded; NO `6+65535` arithmetic |
...
| EC-008 | `length = 254` (first invalid above max) with a valid ADU following in the same segment | `parse_errors++`; `break`; segment discarded; valid following ADU is NOT parsed in this call (it will be delivered again on the next `on_data` or is lost); the break-loop policy favors security over throughput |
...
| `00 01 00 00 00 FD 01 03` (len=253, 0xFD) | 253 | `true` | +0 | regression: maximum valid NOT rejected |
| `00 01 00 00 00 FE 01 03` (len=254, 0xFE) | 254 | `false` | +1 | edge-case: first above maximum |
...
- `src/analyzer/modbus.rs` — `is_valid_modbus_adu`: `h.protocol_id == 0x0000 && h.length >= 2 && h.length <= 253`
...
| **Confidence** | high — [2, 253] range is spec-defined; V1.1b3 harmonized max from 249 to 253 |
