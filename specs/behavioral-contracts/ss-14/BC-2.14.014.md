...
modified:
  ...
  - version: "2.3"
    date: 2026-06-10
    change: "v19 remap: T0855 → T1692.001 per MITRE ATT&CK for ICS v19.0 revocation. All T0855 technique ID references in body updated to T1692.001. Name: 'Unauthorized Command Message' → 'Unauthorized Message: Command Message'. Tactic unchanged: IcsImpairProcessControl. Issue #222; audit: mitre-ics-v19-catalog-audit.md."
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
# BC-2.14.014: Write FC 0x06/0x10/0x16/0x17 in Request Direction Emits Finding Tagged ["T1692.001","T0836"]
...
<!-- Previous version (v1.0): "Write FC 0x06/0x10/0x16 in Request Direction Emits T0836 Modify Parameter Finding"
     ...
     v2.0 model (Decision 13): ONE finding per PDU, mitre_techniques: ["T1692.001","T0836"].
...
per-PDU write Finding for these FCs carries `mitre_techniques: vec!["T1692.001", "T0836"]` —
both "Unauthorized Message: Command Message" (T1692.001) and "Modify Parameter" technique tags on one finding.
...
1. ONE `Finding` is pushed (the same per-PDU finding specified in BC-2.14.013 postcondition 1)
   with `mitre_techniques: vec!["T1692.001", "T0836"]`:
...
2. When the T0831 coordinated-write condition is also met (2nd+ holding-register write within
   the 5-second window), the same finding carries `mitre_techniques: vec!["T1692.001","T0836","T0831"]`
...
1. **Tag set for holding-register writes (authoritative per Decision 13 §13.5, updated per BC-DISCREPANCY-001):**
   FC {0x06, 0x10, 0x16, 0x17} → `mitre_techniques = ["T1692.001", "T0836"]` in the single per-PDU finding.
...
4. FC 0x15 (Write File Record) is in the Write class but is NOT in the T0836 subset.
   It carries `mitre_techniques: ["T1692.001"]` only (targets file records, not holding registers).
   FC 0x17 (Read/Write Multiple Registers) IS in the T0836 subset {0x06, 0x10, 0x16, 0x17}
   and carries `mitre_techniques: ["T1692.001", "T0836"]` — it writes holding registers in its
...
5. **Burst finding is independent:** if the burst threshold is also tripped on this PDU,
   a separate burst Finding with `mitre_techniques: ["T0806","T1692.001"]` is emitted (from
...
|----|-------------|-------------------|
| EC-001 | FC 0x06 with value field = 0xFFFF | Single finding `mitre_techniques: ["T1692.001","T0836"]`; value is forensic evidence captured in evidence string, not a gate. |
| EC-002 | FC 0x10 writing 125 registers (max bulk write) | Single finding with `mitre_techniques: ["T1692.001","T0836"]`; evidence string includes the full ADU byte range. |
| EC-003 | FC 0x16 (Mask Write Register) — AND=0xFFF0, OR=0x000F | Single finding `mitre_techniques: ["T1692.001","T0836"]` (parameter modification via bit-mask; 0x16 is in the holding-register subset). |
| EC-004 | FC 0x10 when `all_findings.len() == MAX_FINDINGS - 1` | Finding with `["T1692.001","T0836"]` would be the (MAX_FINDINGS)th finding — pushed (len was MAX_FINDINGS-1). If T0831 also fires, its tag would be appended to the same finding's vec. No second finding attempted for T0836 separately. |
...
| EC-007 | FC 0x0F (Write Multiple Coils) | NOT in T0836 subset; single finding with `mitre_techniques: ["T1692.001","T0835"]` (coil-write tag set, per BC-2.14.015). No T0836 tag. |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| ADU hex: `00 01 00 00 00 06 01 06 00 10 01 F4` (FC=0x06, UnitID=1) — ClientToServer; 1st write on flow | ONE Finding: `{mitre_techniques=["T1692.001","T0836"]}` pushed; `write_count=1`, `fn_code_counts[0x06]=1` | happy-path (register write, union tag) |
| ADU hex: `00 02 00 00 00 09 02 10 00 00 00 02 04 00 64 00 C8` (FC=0x10, UnitID=2) — ClientToServer | ONE Finding: `mitre_techniques=["T1692.001","T0836"]`; `write_count=1`, `fn_code_counts[0x10]=1` | happy-path (multi-register) |
| ADU hex: `00 03 00 00 00 08 01 16 00 10 FF F0 00 0F` (FC=0x16 Mask Write, UnitID=1) — ClientToServer | ONE Finding: `mitre_techniques=["T1692.001","T0836"]`; `write_count=1`, `fn_code_counts[0x16]=1` | happy-path (mask-write) |
| ADU hex: `00 04 00 00 00 06 01 0F 00 00 00 08 01 FF` (FC=0x0F Write Multiple Coils) — ClientToServer | ONE Finding: `mitre_techniques=["T1692.001","T0835"]`; no T0836 (coil write, not in holding-register subset) | negative (coil write — T0836 not applicable) |
| ADU hex: FC=0x06; 2nd holding-register write within 5s T0831 window — ClientToServer | ONE Finding: `mitre_techniques=["T1692.001","T0836","T0831"]`; T0831 co-tagged inline | happy-path (T0831 co-tag) |
| ADU hex: `00 03 00 00 00 09 01 17 00 00 00 01 00 00 00 01 02 00 42` (FC=0x17 RW-Multiple, UnitID=1) — ClientToServer | ONE Finding: `mitre_techniques=["T1692.001","T0836"]`; `write_count=1`, `fn_code_counts[0x17]=1` (holding-register write; BC-DISCREPANCY-001) | happy-path (RW-multiple, register-write tag) |
| ADU hex: `00 05 00 00 00 06 01 03 00 00 00 05` (FC=0x03 Read Holding Registers) — ClientToServer | No T0836 (Read class); no T1692.001; no finding | negative (read FC) |
...
| MITRE Techniques | T0836 — Modify Parameter (ATT&CK for ICS; IcsImpairProcessControl tactic); T1692.001 — Unauthorized Message: Command Message (always co-tagged; v19 successor to revoked T0855) |
...
- `src/analyzer/modbus.rs` — holding-register write detection branch in `on_data`; sets `mitre_techniques: vec!["T1692.001","T0836"]` for FC {0x06, 0x10, 0x16, 0x17}
...
| Property | Value |
|----------|-------|
| **Path** | f2-fix-directives.md §13.5 (per-PDU emission rules: FC 0x06/0x10/0x16 → ["T1692.001","T0836"]); ADR-006 (multi-tag design); architecture-delta.md §2.6 (T0836 detection trigger: 0x06/0x10/0x16 holding register writes) |
| **Confidence** | high |
