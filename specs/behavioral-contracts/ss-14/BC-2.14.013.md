...
modified:
  ...
  - version: "2.3"
    date: 2026-06-10
    change: "v19 remap: T0855 → T1692.001 ('Unauthorized Message: Command Message') per MITRE ATT&CK for ICS v19.0 revocation; T0855 is REVOKED, successor is ICS sub-technique T1692.001 (parent T1692 'Unauthorized Message'). Tactic unchanged: IcsImpairProcessControl. All technique ID references in body updated. Issue #222; audit: mitre-ics-v19-catalog-audit.md, dnp3-mitre-verification.md."
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
single `Finding` carrying `mitre_techniques: Vec<String>` that includes T1692.001 ("Unauthorized
Message: Command Message") and all other applicable technique tags for that PDU (T0836 for
register writes, T0835 for coil writes). This is the broadest write-class detector: every write
FC triggers exactly one per-PDU finding with a union of applicable technique tags. T0836, T0835,
...
Note: T1692.001 ("Unauthorized Message: Command Message") is an ICS sub-technique under parent
T1692 ("Unauthorized Message") introduced in MITRE ATT&CK for ICS v19.0 (2026-04-28). It
replaces the revoked T0855 ("Unauthorized Command Message"). Technique IDs in ICS may now use
the sub-technique format Txxxx.NNN. Tactic: IcsImpairProcessControl (unchanged).
...
1. Exactly ONE `Finding` is pushed to `self.all_findings` per write-class PDU, with:
   ...
   - `mitre_techniques`: a `Vec<String>` containing ALL applicable technique tags for this PDU:
     - FC in {0x06, 0x10, 0x16, 0x17}: `vec!["T1692.001", "T0836"]` — unauthorized command + modify parameter
       (0x17 Read/Write Multiple Registers writes holding registers; reconciled per BC-DISCREPANCY-001)
     - FC in {0x05, 0x0F}: `vec!["T1692.001", "T0835"]` — unauthorized command + I/O image manipulation
     - FC in {0x15}: `vec!["T1692.001"]` — unauthorized command only (file record write; no register/coil subtype)
...
6. Write-rate window counters are updated and the burst detector runs (per BC-2.14.017).
   ...
   if the rate threshold is exceeded — `mitre_techniques: vec!["T0806", "T1692.001"]`. That burst
...
1. **One finding per write-class PDU** carrying the full union of applicable technique tags.
   There is no separate T1692.001 finding object; T1692.001 is always present in `mitre_techniques`
   of the per-PDU write finding.
2. **Tag union rules** (authoritative, per Decision 13 §13.5):
   - Holding-register writes (FC 0x06, 0x10, 0x16, 0x17): `mitre_techniques = ["T1692.001", "T0836"]`
   ...
   - Coil writes (FC 0x05, 0x0F): `mitre_techniques = ["T1692.001", "T0835"]`
   - File record write (FC 0x15): `mitre_techniques = ["T1692.001"]` (targets file records, not registers/coils)
   - When T0831 window condition is also met (2nd+ holding-register write within 5s):
     `mitre_techniques = ["T1692.001", "T0836", "T0831"]`
...
5. **Burst finding is independent:** the T0806+T1692.001 burst finding (from BC-2.14.017) is a
   separate Finding object emitted alongside (not instead of) the per-PDU write finding. The
   burst finding carries its own `mitre_techniques: vec!["T0806", "T1692.001"]`.
...
|----|-------------|-------------------|
| EC-001 | FC 0x17 (Read/Write Multiple Registers) in request direction | Write class: single finding with `mitre_techniques: ["T1692.001", "T0836"]`. 0x17 writes holding registers -> Modify Parameter (T0836); reconciled with BC-2.14.016 register-write set per BC-DISCREPANCY-001 ruling. |
| EC-002 | FC 0x15 (Write File Record) in request direction | Write class: single finding with `mitre_techniques: ["T1692.001"]` only. Not a register or coil write. |
| EC-003 | FC 0x16 (Mask Write Register) in request direction | Single finding with `mitre_techniques: ["T1692.001", "T0836"]`. 0x16 is in the holding-register subset (BC-2.14.014). |
...
| EC-006 | FC 0x05 (Write Single Coil) in request direction | Single finding with `mitre_techniques: ["T1692.001", "T0835"]`. |
| EC-007 | FC 0x0F (Write Multiple Coils) in request direction | Single finding with `mitre_techniques: ["T1692.001", "T0835"]`. |
| EC-008 | FC 0x06 (2nd holding-register write within T0831 5s window) | Single finding with `mitre_techniques: ["T1692.001", "T0836", "T0831"]`. T0831 tag is included in this finding's vec; no separate T0831 finding object. |
| EC-009 | FC 0x06 tipping the T0806 burst threshold | Per-PDU finding: `["T1692.001", "T0836"]`. Additionally, a separate burst Finding: `["T0806", "T1692.001"]`. Both pushed if cap allows (2 findings total). |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| ADU hex: `00 01 00 00 00 06 01 06 00 10 01 F4` (FC=0x06, UnitID=1) — ClientToServer; 1st write on flow | Single Finding: `{category=Execution, verdict=Likely, confidence=Medium, mitre_techniques=["T1692.001","T0836"]}` pushed; `write_count=1`, `fn_code_counts[0x06]=1` | happy-path (register write) |
| ADU hex: `00 02 00 00 00 06 02 0F 00 00 00 08 01 FF` (FC=0x0F Write Multiple Coils, UnitID=2) — ClientToServer | Single Finding: `mitre_techniques=["T1692.001","T0835"]`; `write_count=1`, `fn_code_counts[0x0F]=1` | happy-path (coil write) |
| ADU hex: `00 03 00 00 00 06 01 06 00 10 01 F4` (FC=0x06) — ClientToServer; 2nd holding-register write within 5s T0831 window | Single Finding: `mitre_techniques=["T1692.001","T0836","T0831"]`; T0831 tag co-included (see BC-2.14.016) | happy-path (T0831 co-tag) |
...
| ADU hex: FC=0x15 Write File Record — ClientToServer | Single Finding: `mitre_techniques=["T1692.001"]` only (no register/coil subtype) | happy-path (file record write) |
| ADU hex: `00 03 00 00 00 09 01 17 00 00 00 01 00 00 00 01 02 00 42` (FC=0x17 RW-Multiple, UnitID=1) — ClientToServer | Single Finding: `mitre_techniques=["T1692.001","T0836"]` (0x17 writes holding registers; BC-DISCREPANCY-001 reconciliation) | happy-path (RW-multiple, register-write union tag) |
...
| MITRE Techniques | T1692.001 — Unauthorized Message: Command Message (always; ICS sub-technique, v19 successor to revoked T0855); T0836 — Modify Parameter (register writes); T0835 — Manipulate I/O Image (coil writes); T0831 — Manipulation of Control (when coordinated-write window met) |
...
- `src/mitre.rs` — `technique_info("T1692.001")` arm; `MitreTactic::IcsImpairProcessControl`
- `.factory/specs/architecture/decisions/ADR-006-multi-technique-finding-attribution.md`
...
| Property | Value |
|----------|-------|
| **Path** | f2-fix-directives.md §13.5 (co-emission model; union tagging); ADR-006 (multi-technique finding design decision); architecture-delta.md §2.6 (T1692.001 detection trigger; per-PDU multi-tag model) |
| **Confidence** | high |
