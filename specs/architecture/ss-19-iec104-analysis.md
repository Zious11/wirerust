...
modified:
  ...
  - date: 2026-07-15
    author: architect
    note: "F-172-201 guard-placement prose precision: detection table T0814 row 'after walk' → 'at on_data entry'; Bounded-Resource Note aligned — guard runs at on_data entry on directional carry alone (equivalent to bounding previous walk's residual). Version bump 1.8→1.9. NOTE: this shard is an input to all 27 BC-2.19.* files — PO must recompute input-hashes for all 27 BCs using bin/compute-input-hash --write."
---
...
| Malformed APCI LEN out of [4,253]: EMIT-WITH-DEDUP one T0814 per flow direction (malformed-LEN dedup flag). Bad start byte: silent resync, no finding. Non-canonical U-frame. Carry-overflow (carry at on_data entry > MAX_IEC104_CARRY_BYTES): one T0814 per flow direction (carry-overflow dedup flag; distinct from malformed-LEN flag). | T0814 Denial of Service | Possible/Likely |
...
first semantics per ADR-013 Decision 2 / WALK-FIRST-RESIDUAL-BOUND): the guard runs at
on_data entry on the directional carry alone — equivalent to bounding the previous walk's
residual (the walk stashes ≤254 bytes by construction). A spec-conformant residual is
