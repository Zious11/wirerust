...
modified:
  - date: 2026-09-06
    actor: spec-steward
    reason: "feature-s7comm F2 INTEGRATE step 2 (POLICY 9 sync with VP-INDEX v2.48 / verification-architecture.md v2.34): VP-048..VP-055 added to VP-to-Module table (S7comm/ISO-on-TCP passive analyzer, SS-20/SS-21, ADR-014). New module rows analyzer/iso_on_tcp.rs (VP-048 Kani, VP-049 Kani, VP-050 proptest; 3 total) and analyzer/s7comm.rs (VP-051 Kani, VP-052/053/054 proptest, VP-055 cargo-fuzz; 5 total) added to Per-Module Coverage Totals. Kani 16→19 (VP-048, VP-049, VP-051); proptest 22→26 (VP-050, VP-052, VP-053, VP-054); cargo-fuzz 3→4 (VP-055, combined iso_on_tcp.rs+s7comm.rs chain, credited to analyzer/s7comm.rs row). Overall Totals 47→55. VP-041 row traces extended +BC-2.18.005, +BC-2.18.006 (Support-enum re-derivation, ADR-014 Decision 3) — no count change (amendment, not new VP). Version bump 1.48→1.49."
  - date: 2026-06-02
    actor: spec-steward
  ...
  - date: 2026-07-13
    actor: architect
    reason: "Feature Mode F2 (feature-iec104, ADR-013): VP-044..VP-047 added — IEC-104 passive analyzer (SS-19, analyzer/iec104.rs). New module row analyzer/iec104.rs added to Per-Module Coverage Totals. Kani 15→16 (VP-044 P0: APCI parse safety (parse_apci_header only)); proptest 20→22 (VP-045 P1: carry-buffer direction isolation; VP-046 P1: classify_frame_format totality); cargo-fuzz 2→3 (VP-047 P1: no-panic fuzz + loop-termination). Overall Totals 43→47. Version bump 1.43→1.44."
  - date: 2026-07-13
    actor: architect
    reason: "F2 Pass-1 adversarial remediation (DF-SIBLING-SWEEP-001, F-H3): VP-044 row in VP-to-Module table scoped to parse_apci_header ONLY — explicit VP-047/VP-046 exclusion notes added: parse_asdu field extraction, N(S)/N(R) counter tracking, and on_data-loop no-panic are covered by VP-047 (cargo-fuzz); classify_frame_format totality over all 256 u8 values is covered by VP-046 (proptest). No VP counts, tool columns, or Totals row values changed (total 47, Kani 16, proptest 22, cargo-fuzz 3, integration/unit 6 UNCHANGED). Version bump 1.44→1.45."
  - date: 2026-07-14
    actor: architect
    reason: "F2 Pass-3 remediation (DF-SIBLING-SWEEP-001, FIX-1/FIX-2): VP-044 BC-scope doctrine codified in VP-INDEX — source_bc expanded to {BC-2.19.001..005} (all parse_apci_header contracts); .006/.015/.026 routing confirmed (VP-047 or not VP-044 source_bc). VP-045 source_bc {025,026} intentional per F2-P3-M3 ruling; PO must add VP-045 forward-anchor to BC-2.19.026. No VP counts, tool columns, Totals row, or module assignments changed in this file. Version bump 1.45→1.46."
  - date: 2026-07-14
    actor: architect
    reason: "F2 Pass-4 remediation (DF-SIBLING-SWEEP-001, F-P4-H1): VP-044 changelog reason corrected — 'APCI parse safety + forward-progress' → 'APCI parse safety (parse_apci_header only)'; NOTE: the strip of forward-progress language from the VP-044 table-row description was NOT completed in this pass (residual survived). No VP counts, tool columns, Totals row, or module assignments changed. Version bump 1.46→1.47."
  - date: 2026-07-14
    actor: architect
    reason: "F2 Pass-5 final remediation (DF-SIBLING-SWEEP-001, F-P5-H2): VP-044 table-row description stripped of residual 'frame-walk forward-progress >= (h.len+2) bytes per iteration (no infinite loop).' sentence; semicolon after 'return None' changed to period. Pass-4 entry corrected to remove false strip-claim. No VP counts, tool columns, Totals row, or module assignments changed. Version bump 1.47→1.48."
---
...
This matrix maps every VP-NNN verification property to its target module and tracks
per-method coverage status. It must remain consistent with VP-INDEX.md (source of truth
for VP enumeration and counts) and `verification-architecture.md` (Provable Properties
Catalog and P0/P1 lists).
...
| VP-044 | APCI length-octet bounds arithmetic safety: parse_apci_header never panics; valid inputs h.len in [4,253] and h.len+2 in [6,255] (no overflow); invalid inputs (data.len()<6, start_byte!=0x68, LEN<4, LEN>253) return None. Pure-core free fn. ADR-013 Decision 8. SCOPE: Kani harness covers parse_apci_header ONLY; parse_asdu field extraction, N(S)/N(R) counter tracking, and on_data-loop no-panic are covered by VP-047 (cargo-fuzz); classify_frame_format totality over all 256 u8 values is covered by VP-046 (proptest). | analyzer/iec104.rs | Kani | P0 | draft |
| VP-045 | IEC-104 carry-buffer direction isolation (RULING-DNP3-SIBLING-001 pattern): proptest_vp045_direction_isolation — interleaved c2s/s2c deliveries produce correct frame_count with carry_c2s/carry_s2c never mixed; parse_errors==0; proptest_vp045_independent_run_equivalence — interleaved run frame_count equals sum of independent same-direction runs; each carry caps at MAX_IEC104_CARRY_BYTES==255 bytes. ADR-013 Decision 3. Mirrors VP-033/VP-035/VP-037. | analyzer/iec104.rs | proptest | P1 | draft |
| VP-046 | classify_frame_format(cf1:u8)->FrameFormat totality: exhaustive + non-overlapping over all 256 CF1 u8 values — bit0==0→IFormat, bits1:0==0b01→SFormat, bits1:0==0b11→UFormat. Pure-core free fn. ADR-013 Decision 4. Mirrors VP-032 Sub-B. | analyzer/iec104.rs | proptest | P1 | draft |
| VP-047 | IEC-104 parser no-panic fuzz (fuzz_iec104_parser): on_data never panics, unwinds, or OOBs on arbitrary byte sequences; carry buffers remain bounded at MAX_IEC104_CARRY_BYTES==255 bytes after any input. ADR-013 Decision 8. Mirrors VP-028. | analyzer/iec104.rs | cargo-fuzz | P1 | draft |
| VP-048 | TPKT header parse safety and four-way totality: parse_tpkt_header never panics or reads OOB for any input; four outcomes (len<4 reject, version≠0x03 reject, length<4 reject, happy-path Some(TpktHeader) for length in [4,65535]) are exhaustive and mutually exclusive; no h.length overflow. Pure-core free fn. ADR-014 Decision 9. Mirrors VP-044/VP-022/VP-032. | analyzer/iso_on_tcp.rs | Kani | P0 | draft |
| VP-049 | COTP header parse safety, TPDU-type exhaustiveness, and protocol-ID verbatim-extraction totality: parse_cotp_header never panics or reads OOB, incl. LI-truncation bounds check; TPDU-type classification over the high nibble is exhaustive/non-overlapping across all 16 nibble values (CR/CC/DT-payload/DT-empty/unrecognized); protocol-ID byte is a total, uninterpreted identity mapping over all 256 u8 values (frozen SS-20→SS-21 boundary). ADR-014 Decision 9. | analyzer/iso_on_tcp.rs | Kani | P0 | draft |
| VP-050 | TPKT/COTP carry-buffer walk-first-residual-bound reassembly, overflow isolation, and 1-byte resync: walk-first anti-evasion (frame-walk extracts every complete frame before any bound is applied to the residual); carry_c2s/carry_s2c never mixed; residual bounded at MAX_S7_ISO_ON_TCP_CARRY_BYTES=65,535 with clear-and-1-per-direction-dedup overflow handling; resync advances exactly 1 byte/iteration (termination guarantee). ADR-014 Decision 8 (WALK-FIRST-RESIDUAL-BOUND). Mirrors VP-045/VP-035/VP-033. | analyzer/s7comm.rs | proptest | P1 | draft |
| VP-051 | S7comm header bounds-before-slice safety: parse_s7comm_header never panics for data.len()<10; caller-side param_length/data_length bounds-check-before-slice enforced (no OOB slice for any adversarial values); header_len+param_length+data_length cannot overflow usize. Pure-core free fn. ADR-014 Decision 9. Mirrors VP-044 (ASDU minimum-length guard). | analyzer/s7comm.rs | Kani | P0 | draft |
| VP-052 | S7comm FC and Userdata-group classification totality: Job/Ack_Data FC match exhaustive over all 256 u8 values; Userdata function-group match exhaustive over all 16 nibble values incl. the load-bearing group-0x03=Block/group-0x07=Time correction (0x03 is Block functions, NOT 0x07); non-vacuity requires the harness to specifically assert 0x03≠0x07 classification, not just "classifies to something". | analyzer/s7comm.rs | proptest | P1 | draft |
| VP-053 | protocol_id four-way dispatch totality and unclassified never-force-fit: on_data's branch on CotpHeader::protocol_id is exhaustive/mutually-exclusive over None / ConnectRequest+ConnectConfirm / Some(0x32) classic / Some(0x72) plus / Some(other)-or-empty-payload unclassified-gap. LOAD-BEARING: all 254 non-{0x32,0x72} values NEVER attributed to S7comm; no proximity-based fallback; sticky first-classification-wins. | analyzer/s7comm.rs | proptest | P0 | draft |
| VP-054 | Program-download/upload structural disjointness: the 0x1A-0x1C Download triad and 0x1D-0x1F Upload triad classify to disjoint S7ClassicFunction variant sets with no shared match arm — never a single collapsed 0x1A..=0x1F range; no cross-triad conflation. Load-bearing for B2's T0843/T0889 MITRE correctness. | analyzer/s7comm.rs | proptest | P1 | draft |
| VP-055 | S7comm/ISO-on-TCP combined parse-chain no-panic fuzz (fuzz_s7comm_parser): full TPKT→COTP→S7comm chain never panics/OOBs on arbitrary byte sequences across arbitrarily many on_data calls; carry buffers remain bounded at 65,535 bytes; frame-walk loop terminates (each iteration extracts a complete frame advancing >=4 bytes, or stashes a residual and returns). Mirrors VP-028/VP-047. | analyzer/iso_on_tcp.rs + analyzer/s7comm.rs | cargo-fuzz | P1 | draft |
...
| analyzer/iec104.rs | 1 (VP-044) | 2 (VP-045, VP-046) | 1 (VP-047) | 0 | 4 |
| analyzer/iso_on_tcp.rs | 2 (VP-048, VP-049) | 1 (VP-050) | 0 | 0 | 3 |
| analyzer/s7comm.rs | 1 (VP-051) | 3 (VP-052, VP-053, VP-054) | 1 (VP-055) | 0 | 5 |
| **Totals** | **19** | **26** | **4** | **6** | **55** |
...
- VP-042 (dispatcher.rs / proptest): draft; lock gate at F6. Authored as part of
  ...
  accumulation properties of the new `unclassified_port_counts: HashMap<(TransportProto, u16), u64>`
  field added to `StreamDispatcher` for the `--coverage-gaps` feature:
  (1) `proptest_vp042_total_count_equals_n` — after N `on_flow_close` calls with
  `DispatchTarget::None`, `unclassified_port_counts.values().sum() == N`;
  (2) `proptest_vp042_per_port_count_equals_frequency` — each per-`(TransportProto, u16)`-key entry equals
  the count of that key in the input sequence (direction-normalized);
  (3) `proptest_vp042_no_count_spurious_on_classified_flows` — `on_flow_close` for flows
  with a non-None `DispatchTarget` does NOT increment any counter.
  IMPORTANT: VP-004 (Kani, `dispatcher::classify()` content-first dispatch, P0, verified)
  MUST be re-run at F6 as a regression confirmation — the `unclassified_port_counts` field
...
- VP-048..VP-055 (analyzer/iso_on_tcp.rs, analyzer/s7comm.rs / Kani + proptest + cargo-fuzz):
  draft; lock gate at F6. Registered feature-s7comm F2 INTEGRATE step 2 (ADR-014), sourced
  from `.factory/cycles/feature-s7comm/f2-verification-delta.md` Parts 1-2 (product-owner
  staging doc). VP-048 (Kani P0) and VP-049 (Kani P0) cover the two SS-20 pure-core parse
  functions (parse_tpkt_header, parse_cotp_header) — mirrors VP-044's Must-Prove treatment
  for the newest passive-analysis subsystem's foundational parsers. VP-050 (proptest P1)
  covers the shared TPKT/COTP carry-buffer walk-first-residual-bound reassembly discipline
  (ADR-014 Decision 8), mirroring VP-045/VP-035/VP-033. VP-051 (Kani P0) covers SS-21's
  S7comm common-header bounds-before-slice safety, mirroring VP-044's ASDU minimum-length
  guard. VP-052 (proptest P1) and VP-053 (proptest P0) cover SS-21's classification totality
  properties — VP-053 is explicitly load-bearing (never-force-fit unclassified-gap handling
  for the port-102 four-way protocol collision) and is P0 despite being a proptest (not
  Kani) property, since the classification state depends on stateful S7commFlowState rather
  than a pure free fn. VP-054 (proptest P1) covers the download/upload structural
  disjointness property feeding B2's MITRE T0843/T0889 correctness. VP-055 (cargo-fuzz P1)
  is the combined TPKT→COTP→S7comm parse-chain no-panic fuzz harness, mirroring VP-028/
  VP-047; its Module column spans both new modules (iso_on_tcp.rs + s7comm.rs) but it is
  credited to the analyzer/s7comm.rs row in Per-Module Coverage Totals to avoid double-
  counting. Two new module rows added: analyzer/iso_on_tcp.rs (2 Kani + 1 proptest = 3
  total) and analyzer/s7comm.rs (1 Kani + 3 proptest + 1 cargo-fuzz = 5 total). VP-041's
  row (src/protocols.rs) traces extended +BC-2.18.005, +BC-2.18.006 (amendment, not a new
  VP; no count change) per ADR-014 Decision 3's Support-enum re-derivation. Grand Totals:
  Kani(19) + proptest(26) + fuzz(4) + integration/unit(6) = 55.
