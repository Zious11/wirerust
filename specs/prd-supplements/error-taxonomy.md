...
| E-INP-008 | Input | `broken` | 1 | `src/reader.rs` (pcapng SHB/IDB parse path) | `Failed to parse pcapng <block-type>: <underlying>` | BC-2.01.010, BC-2.01.011, BC-2.01.017 | Covers SHB **semantic** failures and IDB **body-decode** failures where the crate successfully frames the block but wirerust rejects the content: (a) **SHB semantic failures** — invalid Byte-Order Magic (not `4D 3C 2B 1A` LE or `1A 2B 3C 4D` BE); major_version != 1. (b) **IDB body-decode failures** — `block_total_length` is in [12, 20): the crate frames the block outer header (>= 12 bytes) but the body has fewer than 8 fixed-field bytes (linktype[2] + reserved[2] + snaplen[4]); or malformed options-length in the IDB options section. `<block-type>` is one of "Section Header Block", "Interface Description Block". `<underlying>` is the anyhow root cause. Surfaced via anyhow chain; ultimate wrapper is E-INP-005 ("Failed to read \<path\>: \<underlying\>"). **SCOPE NOTE:** Block-level **framing/length-truncation** (block_total_length < 12; the crate returns Err before wirerust body-decode code runs) is **E-INP-010**, NOT E-INP-008. E-INP-008 applies only when the crate successfully produces a block body that wirerust then rejects semantically. |
