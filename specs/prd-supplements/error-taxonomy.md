...
| E-INP-008 | Input | `broken` | 1 | `src/reader.rs` (pcapng SHB/IDB parse path) | `Failed to parse pcapng <block-type>: <underlying>` | BC-2.01.010, BC-2.01.011, BC-2.01.017 | **Scope (Decision 20):** wirerust **body-decode** failures only — i.e., cases where pcap-file 2.0.0 successfully frames the block (outer header parsed, block body delivered) but wirerust's own decode then rejects it. Two subcategories: (a) **Block body shorter than required fixed-field bytes** — SHB body < 16 bytes (byte-order-magic[4] + major_version[2] + minor_version[2] + section_length[8]); IDB body < 8 bytes (linktype[2] + reserved[2] + snaplen[4]); EPB body < 20 bytes; SPB body < 4 bytes (original_len). (b) **Body semantically invalid** — SHB bad Byte-Order-Magic (not `4D 3C 2B 1A` LE or `1A 2B 3C 4D` BE); SHB major_version != 1. `<block-type>` is one of "Section Header Block", "Interface Description Block". `<underlying>` is the anyhow root cause. Surfaced via anyhow chain; ultimate wrapper is E-INP-005 ("Failed to read \<path\>: \<underlying\>"). **SCOPE BOUNDARY:** Block-level **framing rejection by pcap-file 2.0.0** (block_total_length<12, block_total_length%4!=0, EOF before trailing length field — the crate returns Err before wirerust body-decode code ever runs) is **E-INP-010**, NOT E-INP-008. E-INP-008 applies only when the crate successfully delivers a block body that wirerust then rejects. |
...
| E-INP-010 | Input | `broken` | 1 | `src/reader.rs` (pcapng EPB/SPB/unknown-block parse path; also raw-block framing rejection by pcap-file crate) | `Failed to parse pcapng <block-type> (block #<seq>): <underlying>` | BC-2.01.012, BC-2.01.013, BC-2.01.015, BC-2.01.017 | **Scope (Decision 20):** block **framing rejection by pcap-file 2.0.0** AND wirerust's own EPB/SPB body-decode failures. Specifically: (a) **pcap-file framing rejection (raw-block path, crate owns this check)** — `block_total_length < 12` (minimum valid block size); `block_total_length % 4 != 0` (pcapng alignment requirement); EOF reached before the trailing `block_total_length` field is read. The crate returns Err before wirerust body-decode code runs; wirerust wraps the crate error with the canonical message. (b) **EPB interface_id OOB on a NON-EMPTY interface table** — `<underlying>` is `"EPB interface_id={id} out of range (table size={n})"` (empty-table case is E-INP-009, not this code). (c) **EPB padding-aware over-read** — `captured_len + pad > body_remaining` after the fixed 20-byte EPB header, i.e., the padded packet data extends past the block boundary. (d) **EPB body truncated** (< 20 fixed-field bytes). (e) **SPB body truncated** (< 4 bytes for `original_len` field, i.e., `block_total_length < 16`). (f) **Unknown-block framing errors** (crate returns Err). `<block-type>` is "Enhanced Packet Block", "Simple Packet Block", or "unknown block (type=0x{N:08X})". **`<seq>` convention (M-5):** 1-based; the SHB is block #1; each `next_raw_block` call increments the counter. See the catalog header note. Note: E-INP-008 is RESERVED for SHB/IDB body-decode failures (see that row); it is NOT used for EPB/SPB errors. |
