...
| E-INP-012 | Input | `broken` | 1 | `src/reader.rs` (pcapng SHB re-encounter check) | `pcapng multi-section files are not supported (second Section Header Block at block #<seq>)` | BC-2.01.010, BC-2.01.017 | Emitted when a second SHB is encountered in a stream after the first section's SHB has already been parsed. wirerust supports single-section pcapng only; multi-section pcapng (as generated by some capture rotators) is rejected fail-closed. `<seq>` is the 1-based block sequence number within the file. No byte-order reset is attempted before rejection. In directory mode, this error fails the individual file but does NOT abort the overall run. |
