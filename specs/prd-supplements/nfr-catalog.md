...
| NFR-REL-004 | Reliability | `TcpReassembler::new` defensively asserts all 5 config bounds > 0; panics on invalid config at construction (programmer error only) | Panic only fires on zero-value config; CLI range validators prevent 0 from reaching constructor | Code inspection: mod.rs:115-125 | P1 | N/A | N/A -- src/reassembly/mod.rs:115-125 (5 `assert!` calls in `TcpReassembler::new`) |
