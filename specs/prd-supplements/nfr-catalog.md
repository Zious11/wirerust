...
> v1.3 corrections (2026-06-08): NFR-RES-022 OPEN→CLOSED (dropped_findings implemented); NFR-REL-003 count-gate replaced with property assertion (16 production sites); NFR-MNT-005 inline-test invariant corrected (5 modules, 34 tests); NFR-RES-010 false IDS citation removed; NFR-OBS-001 16→17 counters; NFR-OBS-002 3→4 analyzers; NFR-SEC-002 7→15 tests; NFR-PERF-004 stale "no benchmarks" note updated; stale line refs fixed (REL-001, REL-007, RES-015, MNT-003/004, SUP-001); NFR-RES-024 added for DnsAnalyzer. Validated by research-agent under DF-VALIDATION-001 (report: .factory/research/nfr-validation-report.md).
...
| ID | Category | Requirement | Target | Validation Method | Priority | Risk Source | Status |
|----|----------|-------------|--------|------------------|----------|-------------|--------|
| NFR-REL-001 | Reliability | Release builds use `overflow-checks = true` (in `[profile.release]`); arithmetic overflow panics in release, not just debug | 0 silent wrapping overflows in release | Cargo.toml inspection: `overflow-checks = true` at line 38 | P0 | N/A | N/A -- Cargo.toml:38 confirmed |
...
| NFR-REL-003 | Reliability | Saturating arithmetic used on all adversarial-influenced counter, buffer, and offset math; no wrapping or checked-only ops on attacker-controlled numeric inputs | All arithmetic on attacker-influenced counters / buffer sizes / stream offsets uses `saturating_add` / `saturating_sub`; no unchecked overflow sites on these paths | Property audit: `grep -rn "saturating_" src/` confirms 16 production call sites (17 grep hits minus 1 doc-comment at dispatcher.rs:351); new sites added on any new counter/buffer path must also be saturating | P0 | N/A | N/A -- 16 production saturating call sites confirmed in src/ (dispatcher ×2, reassembly/flow ×1, reassembly/mod ×2, reassembly/segment ×5, analyzer/tls ×2, analyzer/http ×4); all on adversarial counter or buffer depth paths; v1.2 pinned count of 12 was stale and the brittle grep-count gate is replaced by this property assertion |
| NFR-REL-004 | Reliability | `TcpReassembler::new` defensively asserts all 5 config bounds > 0; panics on invalid config at construction (programmer error only) | Panic only fires on zero-value config; CLI range validators prevent 0 from reaching constructor | Code inspection: mod.rs:115-125 | P1 | N/A | N/A -- src/reassembly/mod.rs:115-125 (5 `assert!` calls in `TcpReassembler::new`). **FIX-P5-002 (ADV-IMPL-P04-MED-001, 2026-06-01):** `--reassembly-depth` and `--reassembly-memcap` now use a `parse_nonzero_usize` custom clap `value_parser` that rejects 0 at parse time with a `ValueValidation` error (exit 2, message `"0 is not in 1.."`). This closes the path by which a user-supplied 0 could bypass the CLI layer and reach `TcpReassembler::new`'s assert, causing a panic. The prior panic on operator input is now classified as "prevented operator-input error" — it becomes unreachable on the `analyze` path in production. The asserts remain as programmer-error backstops only. See BC-2.12.005 EC-006/EC-007. |
| NFR-REL-005 | Reliability | `TcpReassembler::finalize` is idempotent: guarded by `self.finalized: bool`; second call is no-op | No double-flush or duplicate summary findings on double-finalize | Code inspection: mod.rs:103 (`finalized` field), mod.rs:615-618 (guard at top of `finalize`) | P0 | N/A | N/A -- implemented; no explicit double-call test yet |
...
| NFR-REL-007 | Reliability | `insert_segment` with no ISN emits ONE process-wide warning via `ISN_MISSING_WARNED: AtomicBool`; returns `InsertResult::IsnMissing`; no panic | Test: `test_isn_missing_returns_isn_missing` passes | Test: segment_tests.rs::test_isn_missing | P0 | N/A | N/A -- AtomicBool guard in segment.rs:16 (`ISN_MISSING_WARNED`), IsnMissing arm with one-shot eprintln at segment.rs:204-207 (v1.2 cited "51-58" — stale line ref; mechanism confirmed at 204-207) |
...
| NFR-REL-009 | Reliability | Decode errors counted into `Summary.skipped_packets`; FIRST error message only printed to stderr | 1 error message per run; no flooding | Code inspection: main.rs:170-177 (decode error branch in capture loop) | P1 | N/A | N/A -- implemented; no test for suppression specifically |
...
| ID | Category | Requirement | Target | Validation Method | Priority | Risk Source | Status |
|----|----------|-------------|--------|------------------|----------|-------------|--------|
| NFR-OBS-001 | Observability | `ReassemblyStats` exposes 17 counters surfaced via `summarize()` into `AnalysisSummary.detail` | All 17 counter fields populated and accessible | Test: `test_summarize_returns_reassembly_stats` | P1 | N/A | N/A -- `ReassemblyStats` struct in src/reassembly/stats.rs:10-31 has 17 fields (lines 11-26 = 16 base fields + `dropped_findings` at line 31); `summarize()` in src/reassembly/mod.rs:706-744 emits 17 detail keys including synthetic `flows_completed`; v1.2 "16 counters" was stale (predated `dropped_findings` at stats.rs:31) |
| NFR-OBS-002 | Observability | Per-analyzer `summarize()` returns uniform `AnalysisSummary{analyzer_name, packets_analyzed, detail: HashMap}` shape | All 4 analyzers + reassembler produce this shape | Tests: per-analyzer summarize tests | P1 | N/A | N/A -- trait enforced at compile time; 4 analyzers: HttpAnalyzer (http.rs), TlsAnalyzer (tls.rs), DnsAnalyzer (dns.rs), + TcpReassembler; v1.2 "3 analyzers" omitted DnsAnalyzer |
...
| NFR-OBS-004 | Observability | Findings carry MITRE ATT&CK technique IDs where a clean mapping exists; `None` preferred over a forced fit | All 15 emitted technique IDs resolve in lookup (current; 17 after STORY-114 — PLANNED); no force-fit | Test: `known_emitted_technique_ids_resolve_in_lookup` | P1 | N/A | N/A -- src/mitre.rs |
| NFR-OBS-005 | Observability | `unclassified_flows` counter from dispatcher injected into reassembly `AnalysisSummary.detail` | Counter visible in JSON/terminal output | Tests: `test_unclassified_flows_counter` | P1 | N/A | N/A -- counter field at src/dispatcher.rs:77 (`unclassified_flows: u64`); accessor fn `unclassified_flows()` at :117-119; incremented at :357 (`self.unclassified_flows += 1` in `on_flow_close` None arm); injected into summary at src/main.rs:209-212 |
...
| NFR-RES-002 | Resource | Per-flow-direction overlap-anomaly alert fires when `overlap_count > overlap_alert_threshold` (default 50); exactly ONCE per direction (sticky latch). Worst-case 6 findings per bidirectional flow (3 alert types x 2 directions) | At most 6 findings per flow from threshold alerts; at most 10,000 total via MAX_FINDINGS | Tests: `test_overlap_anomaly_finding`; latch tests | P0 | N/A | N/A -- default in config.rs:125 (`overlap_alert_threshold: 50`); alert logic in mod.rs:459-479; threshold moved from module const to ReassemblyConfig field per LESSON-P2.05 |
| NFR-RES-003 | Resource | Per-flow-direction small-segment alert fires when `small_segment_run > small_segment_alert_threshold` (default 100); exactly ONCE per direction | At most 1 alert per direction per flow | (No direct test -- BC-RAS-020 P1 planned) | P1 | N/A | N/A -- default in config.rs:126 (`small_segment_alert_threshold: 100`); alert logic in mod.rs:486-517; threshold moved to ReassemblyConfig per LESSON-P2.05 |
| NFR-RES-004 | Resource | Per-flow-direction out-of-window alert fires when `out_of_window_count > out_of_window_alert_threshold` (default 100); exactly ONCE per direction | At most 1 alert per direction per flow | Tests: `test_out_of_window_threshold_alert`; `test_out_of_window_alert_fires_only_once` | P1 | N/A | N/A -- default in config.rs:129 (`out_of_window_alert_threshold: 100`); alert logic in mod.rs:518-542; threshold moved to ReassemblyConfig per LESSON-P2.05 |
...
| NFR-RES-010 | Resource | Default max receive window: `max_receive_window = 1 MB` (1,048,576 bytes); engineering default chosen to bound reassembly memory without rejecting legitimate large-window TCP flows | Out-of-window segments rejected above 1 MB offset | Tests: `test_out_of_window_segment_rejected_by_engine` | P0 | engineering default | N/A -- config.rs:124 (`max_receive_window: 1_048_576`); NOTE: v1.2 cited "matches Suricata/Zeek/Snort default" -- that claim is REMOVED as externally disproven (Snort stream5 default = 0/unlimited, Suricata uses memcap policy, Zeek uses time-based retention); 1 MB is an engineering default, not an industry-standard value |
| NFR-RES-011 | Resource | HTTP per-direction header buffer cap: `MAX_HEADER_BUF = 65,536` bytes; bytes past cap silently dropped | No OOM on HTTP request with header larger than 64 KB | Test: `test_buffer_cap_no_panic_on_oversized_headers` | P0 | N/A | N/A -- http.rs:23 (`const MAX_HEADER_BUF`); cap enforced in `on_data` at http.rs:546 (request), 558 (response) |
| NFR-RES-012 | Resource | HTTP max headers per message: `MAX_HEADERS = 96`; exceeding emits Anomaly/Inconclusive/Medium (T1499.002) | Exactly 1 finding when > 96 headers; 0 findings at exactly 96 | Tests: `test_too_many_headers_generates_finding` | P0 | N/A | N/A -- http.rs:24 (`const MAX_HEADERS: usize = 96`); TooManyHeaders finding emitted at http.rs:435-449 (request) and 496-509 (response) |
| NFR-RES-013 | Resource | HTTP URI list cap: `MAX_URIS = 10,000`; further URIs silently dropped from list (detection still runs) | `uris` list never exceeds 10,000 entries | (No direct test -- BC-HTTP-025 P2 planned) | P2 | N/A | N/A -- http.rs:25 (`const MAX_URIS: usize = 10_000`); cap enforced at http.rs:406 (`if self.uris.len() < MAX_URIS`) |
| NFR-RES-014 | Resource | HTTP/TLS per-map cardinality cap: `MAX_MAP_ENTRIES = 50,000`; new keys silently dropped past cap; existing keys still increment | All 8 aggregate maps bounded at 50,000 keys | Test: `test_non_utf8_sni_finding_fires_when_sni_counts_at_capacity` (TLS); HTTP analogous | P0 | ADR 0002 | N/A -- http.rs:26 (`const MAX_MAP_ENTRIES: usize = 50_000`); tls.rs:31 (same const) |
| NFR-RES-015 | Resource | TLS per-direction record-assembly buffer cap: `MAX_BUF = 65,536` bytes | No OOM on large TLS record stream | (No direct test -- BC-TLS-005 P1 planned) | P1 | N/A | N/A -- tls.rs:30 (`const MAX_BUF: usize = 65_536`); cap enforced via `saturating_sub` in `on_data` at tls.rs:822 (client), 829 (server) |
| NFR-RES-016 | Resource | TLS record payload sanity cap: `MAX_RECORD_PAYLOAD = 18,432`; oversized records increment `parse_errors` and clear buffer (RFC 5246: TLS 1.2 ciphertext max; 18,432 >= TLS 1.3 max of 16,640) | Oversized records never allocated | Test: `test_oversized_sni_exceeds_record_payload_limit` | P0 | RFC 5246/8446 | N/A -- tls.rs:32-34 (`MAX_RECORD_PAYLOAD` const with RFC comment); guard at tls.rs:689 |
| NFR-RES-017 | Resource | HTTP poison threshold: `POISON_THRESHOLD = 3`; empirically calibrated against `http-full.cap` fixture | Exactly 3 consecutive errors required to poison; not 2 | Tests: `test_parse_error_poisons_direction_after_threshold`; `test_single_error_does_not_poison` | P0 | empirical (PR #42) | N/A -- http.rs:82 (`const POISON_THRESHOLD: u8 = 3`) |
...
| NFR-RES-022 | Resource | When MAX_FINDINGS is reached, further pushes are silently dropped; a `dropped_findings: u64` counter on `ReassemblyStats` makes the drop observable via `summarize().detail` | `dropped_findings` counter visible in AnalysisSummary; non-zero when cap was hit | Test: `test_dropped_findings_counter_increments_when_cap_reached` | P1 | N/A | CLOSED -- `dropped_findings: u64` implemented at src/reassembly/stats.rs:31; incremented at mod.rs:477, 515, 539 (three drop sites); surfaced by `summarize()` at mod.rs:738 (`detail.insert("dropped_findings", ...)`); v1.2 status "OPEN -- counter not yet implemented" was incorrect |
...
| NFR-RES-024 | Resource | `DnsAnalyzer` processes only packets where `is_dns_port(src, dst)` == true (port 53 TCP or UDP); payloads shorter than 12 bytes (minimum DNS header length) are classified as responses via the `is_query` length guard; `query_count` and `response_count` are unbounded `u64` accumulators (no per-flow cap needed — analyzer emits zero findings) | `dns_queries` and `dns_responses` counters visible in `AnalysisSummary.detail`; analyzer never emits findings; never panics on short payloads | Test: `test_dns_analyzer_port53_dispatch`; `test_dns_short_payload_no_panic`; `test_dns_summarize_counters` | P1 | N/A | N/A -- `DnsAnalyzer` in src/analyzer/dns.rs; `is_dns_port` at dns.rs:42-45 (src==53 or dst==53); 12-byte length guard in `is_query` at dns.rs:46-53; query_count/response_count accumulated in `analyze()` at dns.rs:70-75; `summarize()` at dns.rs:80-95 emits `dns_queries`/`dns_responses` keys; never-emit-findings contract confirmed (dns.rs:70-75 returns empty vec); added in v1.3 to close catalog gap (DnsAnalyzer was omitted from all NFR-RES/OBS rows in v1.2) |
...
| NFR-MNT-003 | Maintainability | `cargo fmt --all --check`; `rustfmt.toml` pins edition=2024, max_width=100, field-init shorthand, try shorthand | 0 formatting violations in CI | CI job "fmt" | P0 | N/A | N/A -- ci.yml:60-68; rustfmt.toml:1-4 (4 lines; v1.2 cited ":1-5" -- off by one) |
| NFR-MNT-004 | Maintainability | `MitreTactic` is `#[non_exhaustive]` so adding new ATT&CK variants is non-breaking for downstream | Downstream crates must use `_` arm in matches | Compile-time (non_exhaustive attribute) | P1 | N/A | N/A -- src/mitre.rs:46-47 (`#[non_exhaustive]` attribute on `MitreTactic` enum; v1.2 cited 45-47, mechanism confirmed at 46-47) |
| NFR-MNT-005 | Maintainability | Integration and behavioral-contract tests reside under `tests/`; inline `#[test]` modules in `src/` are permitted for unit-testing private helpers in analyzer and reassembly modules where external tests cannot access private state | 34 inline `#[test]` fns across 5 files confirmed (tls.rs: 8, http.rs: 5, mitre.rs: 2, segment.rs: 4, terminal.rs: 15); no expansion of inline testing to new modules without justification | Grep: `grep -rn "#\[test\]" src/`; file count must stay at 5 modules or be justified if new modules are added | P1 | N/A | N/A (OPEN-DEBT note: v1.2 incorrectly stated "zero inline #[test] except terminal.rs (1 allowed module)" -- that invariant was false; 34 inline tests exist across 5 modules; the policy is corrected here to reflect actual convention) |
...
| ID | Category | Requirement | Target | Validation Method | Priority | Risk Source | Status |
|----|----------|-------------|--------|------------------|----------|-------------|--------|
| NFR-SUP-001 | Supply chain | Direct production dependencies: current count per Cargo.toml; unused `rayon` dep declared but not used in any `src/` file | Minimal transitive surface; no unused prod deps | `cargo tree --depth 1 --edges normal`; `grep -r "rayon" src/` | P1 | NFR-VIO-006 | OPEN -- `rayon = "1"` present at Cargo.toml:35 but zero uses in src/ (v1.2 cited line 28 -- stale; current line is 35); NFR-VIO-006 firm-fix disposition (cost: S; remove line); GitHub issue pending |
