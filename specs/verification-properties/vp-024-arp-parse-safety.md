...
kani_version: "0.67.0"  # historical version at verified_at_commit 6e9f2cc; recovered per LMR-002 from in-tree demo-evidence banners (docs/demo-evidence/STORY-106/VP-023-{A,B,C,D}-*.txt line-1 banner "Kani Rust Verifier 0.67.0 (cargo plugin)"; STORY-106/107 evidence-report.md tooling section) and the TOOLING NOTE comment naming Kani 0.67.0 + CBMC in `src/analyzer/tls.rs` at 6e9f2cc; corroborated by contemporaneous factory F6 records (.factory/phase-f6-hardening/kani-results.md environment section; feature-mitre-json-names/f6-hardening.md and feature-enip-v0.11.0/F7-convergence-report.md tool-version rows). PR #250 body confirms Kani was run manually on the F6 machine; CI at 6e9f2cc contains no cargo-kani job (no CI Kani metadata to conflict).
...
modified:
  ...
  - "v2.4 (2026-06-16): E-17 F2 lifecycle append-note (GOVERNANCE ONLY — no proof change). phase remains f6 — the proof-lock phase is immutable; the E-17 append does not re-open the proof phase. E-17 reviewed the stacked-link-extension (QinQ/MACsec) offset path and confirmed it remains within VP-024's parse-safety envelope with no new proof obligation. The QinQ/MACsec offset computation lives in the lax-None arm of `decode_packet` (effectful etherparse lax-parse path) — NOT in `extract_arp_frame` (Sub-A target) or the pure-core functions in `src/analyzer/arp.rs` (Sub-B/C/D targets). Existing coverage is sufficient: cargo-fuzz VP-008 16.2M iterations / 0 panics covers `decode_packet` (including the lax-None ARP arm), and 10 new tests across 2 files cover the offset-formula path that VP-024 Sub-A does not directly exercise: `tests/bc_2_16_qinq_macsec_offset_tests.rs` (4 tests: QinQ behavioral, QinQ model-pin, QinQ malformed→D11, and MACsec observe-only probe `test_BC_2_16_015_macsec_arp_lax_parse_probe` — observe-only, no offset assertion) and `tests/bc_2_16_e17_macsec_offset_tests.rs` (6 tests: `test_BC_2_16_015_macsec_no_sci_unmodified_arp_truncated_offset_22` asserts arp_offset==22, `test_BC_2_16_015_macsec_sci_present_unmodified_arp_truncated_offset_30` asserts arp_offset==30, malformed→D11 for no-SCI/SCI, Modified/opaque-unreachable security guards; branch test/arp-qinq-macsec-fixtures, extends PR #258, committed in F4). The offset==22 and offset==30 arithmetic assertions reside ONLY in `bc_2_16_e17_macsec_offset_tests.rs`; the qinq file's MACsec test is observe-only. No new Kani harness is warranted: the lax etherparse path (`LaxSlicedPacket::from_ethernet`) is an effectful function, not a pure-core function amenable to Kani. No extension to VP-024's proof scope is needed. Lifecycle append to Lifecycle table only. verification_lock, status, proof harnesses, assertions, and all sub-property content are unchanged."
  - "v2.5 (2026-07-09): STORY-161 FU-F6-KANI-CLEANUP discharge (GOVERNANCE ONLY — no proof content changed). (1) proof_file_hash populated with 48296b21a5bbce59750e6210da8d55be8bf7d3d4a1ed6719088dd4ef59a2c8a5 (full 64-char lowercase hex SHA-256), computed per the mini-Merkle algorithm codified in VP-INDEX v2.39 (Multi-File Proof Anchor Algorithm): sha256( sha256_raw(LF-normalized S(fileA)) || sha256_raw(LF-normalized S(fileB)) ), where fileA = src/analyzer/arp.rs and fileB = src/decoder.rs per the normative module: field order (arp.rs listed first). Extraction snapshot: commit 6e9f2cc via `git show` (EC-002 snapshot rule applied — 6e9f2cc is a merge commit not reachable from develop's squash-linear history). Per-file extraction: fileA arp.rs kani_proofs section byte range 191998..196735 inclusive, raw section length 4738 bytes; fileB decoder.rs kani_proofs section byte range 29801..37992 inclusive, raw section length 8192 bytes. Zero CR bytes in either file; LF-normalization was a no-op (normalized length equals raw length in both files). Per-file digests: sha256_A = c9e6414f92ea6ea50d31dcad870693b65d48b69ea9989dd8807518b2cd493e95 (arp.rs); sha256_B = 1e7370ae91660caa82a7301a07f32bfe7ee08a46ccb35cff9965d2787d7d4049 (decoder.rs). Section-Scoping Rule (VP-INDEX v2.39) verified: exactly one `#[cfg(kani)]` and exactly one `mod kani_proofs` in each file; balanced-brace pairing from the `{` after `mod kani_proofs` located the matching closing `}` at the recorded end offsets; no trailing bytes captured. Dual-tool independent verification: (a) Python `hashlib` script driven by `python3 /tmp/vp024/extract.py` (canonical hashlib.sha256 with byte-level LF normalization and balanced-brace section extractor); (b) Bash pipeline using `grep -bo` for offset location, `perl -e` (binary-safe) for balanced-brace end-offset and LF normalization, `dd bs=1 skip=<start> count=<len>` for byte extraction, `shasum -a 256` for per-file hashing, and `{ printf '%s' \"$digA_hex\" | xxd -r -p; printf '%s' \"$digB_hex\" | xxd -r -p; } | shasum -a 256` for the mini-Merkle raw-digest concatenation and final hash. Both tools produced identical sha256_A, sha256_B, and final hash; a third independent orchestrator recomputation also agreed. Per AC-161-003 dual-tool-discipline mandate and LMR-001 first-write-permanence, all three implementations agreed before the field was written. (2) kani_version populated with '0.67.0' as a sibling of proof_file_hash/verified_at_commit per LMR-003 (Locked-Doc-Appendable Provenance Field Allowlist — kani_version is the sole currently-allowlisted provenance field; field was absent at lock time; sibling placement; no integrity-anchor value; no proof content change; verification_lock NOT cleared). Value sourced per LMR-002 historical recovery: (a) demo-evidence transcripts committed IN the tree of 6e9f2cc (docs/demo-evidence/STORY-106/VP-023-{A,B,C,D}-*.txt line-1 banner \"Kani Rust Verifier 0.67.0 (cargo plugin)\"; STORY-106/107 evidence-report.md tooling sections); (b) a TOOLING NOTE source-code comment in src/analyzer/tls.rs at 6e9f2cc naming \"Kani 0.67.0 + CBMC\"; (c) contemporaneous factory F6 hardening records (.factory/phase-f6-hardening/kani-results.md environment section; feature-mitre-json-names/f6-hardening.md and feature-enip-v0.11.0/F7-convergence-report.md tool-version rows) all naming cargo-kani 0.67.0 as the same-machine installed version across the 2026-06-09..2026-06-16 F6 sweep. PR #250 body confirms Kani was run manually on the F6 machine; CI at 6e9f2cc contains no cargo-kani job (no CI Kani metadata to conflict). Honest-unknown fallback was NOT required — the historical version is unambiguously recoverable from tree-committed evidence at 6e9f2cc itself. Recording the currently-available GitHub release was explicitly avoided per LMR-002. (3) FU-F6-KANI-CLEANUP marker resolved per LMR-001 (Deferred-Null Anchor First-Population): the `proof_file_hash: null  # No canonical recomputation method defined...` line was a sentinel-absent null with an explicit deferral comment recorded in the v2.0/v2.1 modified log at lock time; the null→computed-hash first-write is a completion of the original lock intent, not a mutation of an established anchor; no unlock ceremony was performed. verification_lock: true remains set throughout this story; status, proof_completed_date, phase, harness code, postconditions, property statements, and BC anchor table are unchanged. (4) Bundled template-conformance hygiene: `inputs: []` and `input-hash: \"d41d8cd\"` (MD5 of empty bytes, the canonical empty-inputs value per CLAUDE.md `PG-HASH-EMPTY-INPUTS`) added to satisfy the L4-verification-property template validator (recent template drift required these fields; VP-024 was locked before the requirement existed; empty `inputs:` reflects that VP-024 is a governance-only doc with no separate spec-input files driving its content; the input-hash was computed via the canonical `bin/compute-input-hash --write` tool). Bundled TD-031 stable-anchor hygiene: two pre-existing volatile `decoder.rs:NNN-NNN` line citations (in the v1.8 modified log entry and the Property Statement point 3) were migrated to the stable anchor `` `decoder.rs` fn `extract_arp_frame` type/size-rejection guard (per D-077) `` per TD-VSDD-091; two additional identical citations inside the Sub-A negative-harness code-comment block received the same replacement. This is a citation-form-only change (identical semantic reference), analogous to the VP-INDEX v2.35 `TD-031 stable-anchor remediation (FIX-D)` precedent that retroactively migrated cites in modified-log entries and prose. It was required to unblock the primary STORY-161 amendments (the validate-stable-anchors hook performs whole-file scanning; pre-existing violations blocked all edits) and does not alter any property, postcondition, harness code semantics, or BC anchor. (5) Version bump 2.4→2.5; verification_lock: true throughout; no proof re-run; no harness code change; no property statement semantics changed."
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
3. When `hw_addr_type() != ETHERNET` or `proto_addr_type() != IPV4` or `hw_addr_size() != 6`
   ...
   `decoder.rs` fn `extract_arp_frame` type/size-rejection guard (per D-077)), the function returns `None` (no panic — graceful rejection). This is
...
mod kani_proofs {
    ...
    // i.e., when ANY of the four conditions at `decoder.rs` fn `extract_arp_frame` type/size-rejection guard (per D-077) is true:
    ...
    fn verify_extract_arp_frame_none_on_bad_size() {
        ...
        let hlen = buf[4];
        let plen = buf[5];
        // Restrict to the full rejection region — mirrors the guard at `decoder.rs` fn `extract_arp_frame` type/size-rejection guard (per D-077):
...
| E-17 governance review (APPEND-ONLY — no proof change) | 2026-06-16 | architect |
