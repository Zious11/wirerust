...
mod vp037_modbus_carry_direction_isolation {
    ...
    proptest! {
        ...
        fn proptest_vp037_direction_isolation_fn_code_counts(
            ...
            // Must be in 1..7 (partial MBAP header, not yet complete 8-byte minimum).
            split_offset in 1usize..7,
        ) {
...
- The `split_offset in 1..7` range ensures the first delivery is always a partial MBAP
  header (not a complete 8-byte minimum), forcing the carry-stash path.
...
2. **Bounded input space:** `split_offset` ranges over 1..7 (6 values). The proptest
