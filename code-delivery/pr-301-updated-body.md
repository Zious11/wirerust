...
| `pcapng-many-interfaces.pcapng` | `"Unsupported pcap link type: NULL"` | DLT_NULL(0) loopback — out of scope |
...
### Local Gate (run 2026-06-22, commit `dd79c59`)
...
- `pcapng-many-interfaces.pcapng`: DLT_NULL(0) link type is unsupported — `"Unsupported pcap link type: NULL"` clean rejection, no panic
...
N/A — test-only maintenance addition (one updated test file, 343 lines, no production code). Adversarial review evaluated at Phase 5 for FE-001 cycle (all 3 passes SATISFIED, recorded in STATE.md).
...
Test-only diff: one updated file `tests/e2e_corpus_smoke_tests.rs` (196 lines → 343 lines, +195 lines -49 lines net). No production code, no new dependencies, no network calls, no I/O beyond reading local gitignored fixture files. Attack surface delta: zero.
...
- [x] Local gate (dd79c59): 29 verified, 0 mismatch, 0 panic; clippy/fmt/test all green
