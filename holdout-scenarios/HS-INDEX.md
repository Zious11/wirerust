...
**Repaired (wave-72-repair-2026-07-09) — 13 stale JSON-assertion expectations corrected:**
- **HS-021, HS-032, HS-034, HS-059, HS-065, HS-074** (primary list): verdict/confidence/category
  PascalCase literals lowercased to spec-sanctioned JSON shape per BC-2.11.036 v1.2
  (lowercase verdict, confidence; snake_case category). All 6 version-bumped, modified: logs added.
- **HS-064, HS-075** (primary list): 5-key JSON envelope expanded to 6-key (adds `schema_version`)
  per BC-2.11.037 six-key envelope with `schema_version: "2"`. jq 'keys' assertions, BC-linkage
  tables, evaluation rubrics, and Failure Guidance updated throughout both files. Version bumped 1.1→1.2.
- **HS-024, HS-033, HS-035, HS-050, HS-054** (DF-SIBLING-SWEEP-001 additional hits): category
  literals `"Anomaly"`→`"anomaly"`, `"Reconnaissance"`→`"reconnaissance"` (HS-024); `"Anomaly"`
  (HS-033, HS-054); confidence `"Low"`→`"low"` (HS-035); `"Medium"`→`"medium"` and `"Low"`→`"low"`
  (HS-050). All 5 were v1.0→v1.1 with modified: logs. These are stale expectations only —
  holdout-evaluator scored all affected scenarios 1.00 against current spec-sanctioned behavior.
- **Leave-unchanged verdicts (DF-SIBLING-SWEEP-001):**
  - HS-006:96 `"Likely"` — terminal display token context ("Uppercase is strict"); NOT a JSON assertion.
  - HS-008:75 `"Reconnaissance"` — terminal tactic header in kill-chain order display; NOT a JSON category.
  - HS-082:76 `"Likely"`, `"Inconclusive"` — terminal output label check (--no-color); NOT JSON assertions.
  - BC-linkage table "Anomaly/Likely/High" shorthand in HS-032:64, HS-074:61,63,64 — BC postcondition
    descriptions, not JSON field assertions; left unchanged per "when in doubt, leave it and report" rule.

**Repaired (maint-2026-07-06 FIX-C) — 4 scenarios restored to `active` (previously stale per Sweep 4):**
- **HS-061** (BC-2.06.023): asserts "Exactly 9 keys" in HTTP analyzer detail map. Product now
  emits 10 keys — `dropped_map_entries` added in v0.11.4 via BC-2.06.023 v1.6 (silent-limit
  audit; new-key drops on methods/hosts/user_agents at MAX_MAP_ENTRIES=50,000).
- **HS-064** (BC-2.11.001..005): asserts "Exactly 3 top-level keys" (`summary`, `findings`,
  `analyzers`) in JSON reporter output. Product emits 5 top-level keys since PR #209 —
  adds `mitre_attack_version` and `mitre_domain` (ATT&CK-for-ICS v19.1 envelope). Previously
  flagged as FAIL-STALE in maint-2026-06-22 sweep; still unfixed at v0.11.4.
- **HS-066** (BC-2.07.031): asserts "exactly 7 keys" in TLS analyzer detail map. Product now
  emits 10 keys — `handshake_reassembly_overflows` (BC-2.07.039), `buffer_saturation_drops`
  (BC-2.07.043), and `dropped_map_entries` (BC-2.07.031 v1.5, added in v0.11.4).
- **HS-075** (BC-2.11.001..002): asserts "Exactly 3 top-level keys" for the JSON reporter
  (same schema as HS-064). Same product state, same remediation path.

Resolution (FIX-C, maint-2026-07-06): assertion text and verification approach updated in all four scenario files. Exact key sets enumerated from authoritative BC versions. Scenario semantics unchanged — same behaviors tested, only expected output shapes corrected.
...
|-------|-------|----------|----------------|-----|
| [HS-123](HS-123-protocols-partition-counts-and-filter-flags.md) | `protocols` Subcommand — Partition Counts (8+22=30) and Filter Flag Semantics | P0 | No | BC-2.18.003, BC-2.18.004, BC-2.12.022 |
| [HS-124](HS-124-protocols-goose-powerlink-ethertype-canonical.md) | `protocols` Terminal — GOOSE 0x88B8 (35000), POWERLINK 0x88AB (34987), EtherCAT 0x88A4 (34980), PROFINET-DCP 0x8892 (34962), SV 0x88BA (35002) EtherType Canonical Values | P0 | **YES** (IEC 61850-8-1 §4; IEEE RA; IEC 61158-3-12; IEC 61158-4-10) | BC-2.18.001, BC-2.18.003 |
...
| BC-2.18.001 (EtherCAT ethertype display: 0x88A4 = 34980) | HS-124 Case F | IEEE RA "EtherCAT Protocol" (ETG/Beckhoff); IEC 61158-3-12; ETG.1000.6 §2 |
| BC-2.18.001 (PROFINET-DCP ethertype display: 0x8892 = 34962) | HS-124 Case G | IEEE RA 0x8892 (Siemens AG / PI); IEC 61158-4-10; Wireshark ETHERTYPE_PROFINET |
| BC-2.18.001 (SV ethertype display: 0x88BA = 35002 — guards GOOSE/SV transposition) | HS-124 Case E | IEC 61850-8-1 §4 (SV distinct from GOOSE by EtherType + 2); IEEE RA |
...
> BCs covered: BC-2.19.029, BC-2.19.030, BC-2.19.022 (v1.1 narrowing), BC-2.19.017, BC-2.19.019.
> Epic: feature-iec104. Risk source: IEC104-TIMED-CMD-GAP-001 (CONFIRMED/HIGH confidence).

| HS ID | Title | Priority | Category | BCs |
|-------|-------|----------|----------|-----|
| [HS-133](HS-133-iec104-timed-switching-cmds-t1692001.md) | IEC-104 Time-Tagged Switching Commands (TypeIDs 58–60) Emit T1692.001 | P0 | security-probes | BC-2.19.029, BC-2.19.017 |
| [HS-134](HS-134-iec104-timed-setpoint-bitstring-t1692001-t0836.md) | IEC-104 Time-Tagged Set-Point + Bitstring Commands (TypeIDs 61–64) Emit T1692.001 + T0836 | P0 | security-probes | BC-2.19.030, BC-2.19.017 |
| [HS-135](HS-135-iec104-timed-parity-neighbor-silence-guard.md) | IEC-104 Timed/Untimed Parity and Neighbor Silence Regression Guard (TypeIDs 52–57 and 65–99 Stay Silent) | P0 | edge-case-combinations | BC-2.19.029, BC-2.19.030, BC-2.19.022, BC-2.19.019, BC-2.19.017 |
| [HS-136](HS-136-iec104-timed-control-real-world-corpus.md) | IEC-104 Timed Control Commands — Real-World Corpus Validation (4SICS Known-Good + ICS-PCAP Attack Corpus) | P0 | real-world-corpus | BC-2.19.029, BC-2.19.030, BC-2.19.022 |
