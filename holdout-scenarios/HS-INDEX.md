...
**Stale (maint-2026-07-06 Sweep 4) — 4 scenarios marked `stale` per DF-030 lifecycle:**
...
Remediation: product-owner update to relax "exactly N keys" assertions to explicit key-set
membership plus the observability-counter extensions. Frontmatter-only edit in this sweep;
scenario bodies unchanged; input-hashes pre-existing drift not touched (state-manager owns
hash refresh).
