...
level: ops
version: "1.2"  # Pass-4 R4 C-1 / ADR-009 rev 7: added Case E — captured_len NOT a multiple of 4 at the body boundary (captured_len ≡ 3 mod 4, where captured_len passes <= btl-32 but captured_len + padding would exceed the body) → Err E-INP-010, no panic/OOB. Exercises the padding-aware bound check. Prior: v1.1 O-1 re-cited BC-2.01.012 Postcondition references for interface_id cases. ADR-009 rev 6.
status: draft
...
### Case E — captured_len NOT a multiple of 4, padding would exceed body → E-INP-010
...
implementation must return **E-INP-010** — not proceed to an out-of-bounds slice or panic.
...
- wirerust MUST return **E-INP-010**. No panic. No out-of-bounds slice access.
...
3. The tool exits non-zero. An error is printed to stderr. No allocation of a 15-byte
   ...
   Error consistent with **E-INP-010**.
...
| BC-2.01.012 | Postcondition 6 — EPB captured_len > block_total_length - 32 → E-INP-010 | Case D: btl-31 MUST be rejected (one byte over the valid boundary) |
| BC-2.01.012 | Postcondition 6 — EPB padding-aware bound: captured_len passes raw check but padded extent exceeds data zone → E-INP-010 | Case E: captured_len ≡ 3 mod 4, raw check passes, padded size overflows; no panic/OOB |
...
echo "Exit: $?"
```
Expect: non-zero exit, error on stderr consistent with E-INP-010 (padding-aware bound
exceeded; or structurally invalid block_total_length not a multiple of 4). No JSON on
...
- Case E isolates the padding-aware bound: a captured_len that passes the raw
  ...
  evaluator accepts either a "padding overflow" or "block length not 4-aligned" error
  message — both are E-INP-010 class responses indicating a structural malformation.
...
Case E exit 0 or panic means the padding-aware bound is absent: the guard checks raw captured_len but does not account for ceil(captured_len/4)*4 (padded extent). A 15-byte slice with 1 pad byte reads 16 bytes from a 15-byte zone — silent OOB or panic.
