...
canonical_spec_citation: "GOOSE EtherType 0x88B8 = 35000 decimal per IEC 61850-8-1 §4 and IEEE RA EtherType registry 'IEC GOOSE'; POWERLINK EtherType 0x88AB = 34987 decimal per IEEE RA registry 'ETHERNET Powerlink' (EPSG assignment), Wireshark ETHERTYPE_EPL_V2, IETF ietf-ethertypes value 34987."
input-hash: "b94b093"
---

# Holdout Scenario: `protocols` Terminal — GOOSE and POWERLINK EtherType Canonical Values (DF-CANONICAL-FRAME-HOLDOUT-001)
...
- **GOOSE canonical value** (weight: 0.35): Case A: `0x88B8 (35000)` exactly in GOOSE row.
  This is a CANONICAL-VALUE must-pass: wrong EtherType means catalog data is corrupted.
- **POWERLINK canonical value** (weight: 0.30): Case B: `0x88AB (34987)` in POWERLINK row.
  Wrong value (e.g., `0x3E3F`) means the obsolete V1 value was used.
- **ARP dash** (weight: 0.15): Case C: ARP EtherType is `—` (ethertype: None).
- **[L2] indicator** (weight: 0.10): Case D: all 5 LinkLayer entries show `[L2]`.
- **SV distinct from GOOSE** (weight: 0.10): Case E: SV has `0x88BA (35002)` not `0x88B8`.
