...
| STORY-139 | EC-X1/EC-X2 direction-carry + saturating-window fix (BC-2.17.016 v2.0 + BC-2.17.008 v1.3 + BC-2.17.012 v1.2 + BC-2.17.018 v1.1; VP-033 + VP-034) | — | — | STORY AUTHORED (F3, 2026-06-27) — F4 pending | Per-direction carry split (carry_c2s/carry_s2c), on_data direction threading (Modbus pattern), resolve_enip_client_ip → direction-based src_ip, wrapping_sub → saturating_sub (3 windows), T0814 >= 300 → > 300 (EC-X4 pin), malformed_window_start → malformed_window_start_ts (F-006 rename). VP-033 proptest (carry direction isolation) + VP-034 proptest (window monotonic). Wave 62 (dep=STORY-138). Release blocker per RULING-EDGECASE-001. input-hash 759464a. |
