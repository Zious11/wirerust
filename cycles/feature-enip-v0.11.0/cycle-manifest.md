...
| F2 — Spec Evolution | ADVERSARIAL CONVERGENCE 0/3 — Pass 9 FAIL (REMEDIATED); Pass 10 running | D-229: UDP/2222 deferred to v0.12.0. Scope now TCP/44818 + CIP ForwardOpen (TCP only). 25 BCs total (BC-2.17.001..025). ADR-010, VP-032 written. F2 consistency audit: 7 findings fixed. Pass 1 (2026-06-24): FAIL 4C/7H/3M/3L — LE endianness, T0846, frame-skip soundness, BC-2.17.025 added, BC-INDEX v1.75. Pass 2 (2026-06-24): FAIL 4C/3H/3M/2L — ADR EMITTED 17→20, VP-032 LE, BC-007/008/009/015/018, PRD 24→25 BCs. Pass 3 (2026-06-24): FAIL 3C/4H/4M — dominant pattern propagation-lag; 11 adversary + 3 bonus BE residues REMEDIATED via exhaustive sweep. Pass 4 (2026-06-24): FAIL 0C/1H/4M/2L — anchor/RTM/capability layer residues (cli.rs SS-12 anchor, VP-INDEX:8 harness names, PRD O-04 T1693.001, CAP-17 BC range/OA-001, RTM BC-2.10.005 seed counts) all REMEDIATED. Pass 5 (2026-06-24): FAIL 0C/1H/3M/1L REMEDIATED — ARCH-INDEX EMITTED 17→20/catalogue-only 8, SS-17 BC count 24→25, PRD §6.5 T0846 emitted, MALFORMED_ANOMALY_THRESHOLD anchor re-pointed. ARCH-INDEX v1.7→v1.8. Pass 6 (2026-06-24): PASS 0C/0H/2M/1L — FIRST CLEAN PASS. F6-01 BC-2.17.025 pdu_count anchor corrected; F6-02 BC-2.17.014 error-burst `>` off-by-one reconciled; F6-03 OA-001 label hygiene fixed. All REMEDIATED. Pass 7 (2026-06-24): FAIL 0C/1H/0M/1L REMEDIATED — F-P7-001 (HIGH): ADR-010 Decision 4 doc-comment reworded to strict `>` / "6th error fires; 5th does not"; OA-005 line 676 same. F-P7-002 (LOW): constant value 5 unchanged (correct). 8/9 axes CLEAN. Severity trajectory: 4C/7H→4C/3H→3C/4H→0C/1H→0C/1H→0C/0H→0C/1H→0C/0H. Convergence counter RESET to 0/3 (streak restarting). Pass 8 (2026-06-24): PASS 0C/0H/0M/3L — ALL 9 AXES CLEAN. 3 LOW deferred pre-F3 (non-blocking). Convergence counter: 1/3. Content FROZEN for Pass 9. Pass 9 (2026-06-24): FAIL 0C/1H/1M/2L-obs — genuinely new protocol-correctness bug (NOT propagation-lag): F-P9-001 (HIGH) 0x00B1 connected-item sequence-count offset bug in BC-2.17.006 (all CIP request detections misparse by 2 bytes for 0x00B1 items); REMEDIATED via Option A: CIP service detection scoped to 0x00B2 unconnected carriers for v0.11.0; 0x00B1 connected-item CIP request detection DEFERRED to v0.12.0 (mirrors BC-2.17.008 response gate). F-P9-002 (MEDIUM) parse_cip_header/parse_cpf_items not covered by VP-032 Kani; REMEDIATED via ADR-010 Decision 8 DEFERRED F6 cargo-fuzz obligation + architecture-delta §4.3 note. Convergence counter RESET to 0/3. Pass 10 RUNNING. Severity trajectory: 4C/7H→4C/3H→3C/4H→0C/1H→0C/1H→0C/0H→0C/1H→0C/0H→0C/1H. NOTE FOR F2 HUMAN GATE: Pass 9 remediation is a scope reduction — 0x00B1 connected-item CIP request detection (Stop/Reset/write/Identity) is DEFERRED to v0.12.0; v0.11.0 detects CIP request operations only on 0x00B2 unconnected carriers. Human confirmation required at F2 gate. |
...
| F8-01 | 0x4B/GetAndClear labeled "firmware download marker" + T1693.001 — ODVA grounding is a wirerust convention, not normative common service. Add citation/vendor-specific note at F4. (realism) | OPEN — resolve or defer before F3 |
| F8-02 | [process-gap] ADR-010 Decision 4 specifies EnipFlowState but never sketches the EnipAnalyzer aggregate struct (error_count, write_count, total_pdu_count, enip_write_burst_threshold, all_findings). Add an EnipAnalyzer struct sketch to ADR-010/architecture-delta before F4 implementation. | OPEN — resolve before F3 |
| F8-03 | BC-2.17.014 should state total_error_count = flow.error_counts_in_window.values().sum() (across all status codes, not per-code). One-sentence clarification. | OPEN — resolve or defer before F3 |

## F2 Deferred LOW Polish (pre-F3 required)

Pass 8 PASS (2026-06-24). Three LOW findings deferred as non-blocking. Must be resolved or explicitly
re-deferred with justification before F3 entry (S-7.02).

| ID | Finding | Fix Required | Process Gap? |
|----|---------|-------------|--------------|
| F8-01 | 0x4B/GetAndClear labeled "firmware download marker" + T1693.001 — ODVA grounding is a wirerust convention, not normative common service. | Add citation/vendor-specific note to architecture-delta at F4. (realism) | No |
| F8-02 | ADR-010 Decision 4 specifies EnipFlowState but never sketches the EnipAnalyzer aggregate struct (error_count, write_count, total_pdu_count, enip_write_burst_threshold, all_findings). | Add EnipAnalyzer struct sketch to ADR-010/architecture-delta before F4 implementation. | Yes — [process-gap] ADR struct-completeness |
| F8-03 | BC-2.17.014 should state total_error_count = flow.error_counts_in_window.values().sum() (across all status codes, not per-code). | One-sentence clarification in BC-2.17.014 body. | No |
