...
pipeline: FEATURE
phase: F2
phase_status: "F2 adversary pass-7: 1C/3H/4M/4L NOT clean; clean-pass 0/3; novelty MODERATE, 2 axes converged (SPB body.len()-4 arithmetic; VP-INDEX self-consistency), findings localized to OPB-notice subsystem (rev-8/9 propagation lag) + symbol-rename lag; trajectory 23/24/17/13/13/13/12; remediation round-7 pending."
product: wirerust
mode: brownfield
...
## Session Resume Checkpoint (2026-06-20 — F2 PASS-7 NOT CLEAN (D-158) / REMEDIATION ROUND-7 PENDING / F3 BLOCKED)

**Previous checkpoint (F2 PASS-6 RE-AUDIT MINORS FIXED (D-157) / PASS-7 PENDING / F3 BLOCKED) archived to:
`.factory/cycles/feature-pcapng-reader/session-checkpoints.md`**

### PIPELINE STATUS: FEATURE MODE — F2 PASS-7 NOT CLEAN (D-158): 1C/3H/4M/4L; NOVELTY MODERATE; 2 AXES CONVERGED (SPB FORMULA / VP-INDEX); FINDINGS LOCALIZED TO OPB-NOTICE SUBSYSTEM + SYMBOL-RENAME LAG; TRAJECTORY 23/24/17/13/13/13/12 (CONVERGENCE APPROACHING); CLEAN-PASS COUNTER 0/3; REMEDIATION ROUND-7 PENDING; F3 BLOCKED

Active cycle: **feature-pcapng-reader**. F2 pass-7 NOT CLEAN (D-158): 1C/3H/4M/4L; novelty MODERATE (down from HIGH); 2 axes CONVERGED (SPB body.len()-4 / VP-INDEX). Trajectory 23/24/17/13/13/13/12; convergence approaching. Pass-7 findings cluster: OPB counter model contradiction (F-1 CRITICAL: BC-2.01.015 "both counters" vs HS-108 Cases D/E subset violation); stale field name in HS-108 (F-2 HIGH: `obsolete_packet_blocks` → `opb_skipped` rename missed 6×); notice display arithmetic undefined for mixed-skip case (F-3 HIGH: BC-2.01.009 PC6 lacks subtraction convention); EPB decode precedence unpinned + Precondition 1 contradiction (F-4 HIGH); symbol-rename lag in HS-107/BC-2.01.013 (F-5/F-7 MED); HS-104/107/108 input-hash "tbd" (F-6 MED process-gap); BC-2.01.013 missing btl=14 alignment fixture (F-8 MED). Remediation round-7 pending. STORY-128 + STORY-127 scoped for F3. No in-flight story worktrees. No open PRs. **BEHAVIORAL DECISIONS TO SURFACE AT F2 HUMAN GATE: Decision 15 (interleaved-IDB reject → E-INP-013); Decision 16 (per-SHB reset dead-spec deferred); Decision 17 (IDB-parse precedence order); Decision 19 (zero-packet notice gating — amended rev 8: emission from main.rs, canonical format); Decision 20 (uniform block error-code rule); Decision 21 (if_tsoffset out-of-scope); Decision 22 (canonical spb_data_available=body.len()-4).**

### A. EXACT POSITION

- **Status:** FEATURE mode — pcapng reader cycle open. F2 adversarial reconvergence in progress (pass-6 REMEDIATED — D-156; adversary pass-7 pending). F3 BLOCKED.
...
- **Spec versions:** prd.md v1.33, error-taxonomy v3.4 (next_free E-INP-014; unchanged), nfr-catalog v2.3, ADR-009 rev 9 (Decision 22: canonical spb_data_available=body.len()-4; HS-104 interface_id discriminant split), VP-INDEX v2.8 (total 31; VP-027 discriminant extension + VP-031 formula corrected), BC-INDEX v1.63, BC-2.01.009 v1.6, BC-2.01.010 v2.1, BC-2.01.011 v1.6, BC-2.01.012 v1.6, BC-2.01.013 v1.6, BC-2.01.014 v1.5, BC-2.01.015 v1.7, BC-2.01.016 v1.4, BC-2.01.017 v1.6, BC-2.01.018 v1.6, BC-2.12.011 v1.5, HS-103 v1.5, HS-104 v1.4, HS-107 v1.5, HS-108 v1.3, HS-INDEX v2.3 (all-namespace=181), verification-architecture v2.4, verification-coverage-matrix v1.18. STORY-001 v1.6, epics.md v1.6, test-vectors v2.2.
...
**FEATURE MODE — pcapng reader cycle OPEN (feature-pcapng-reader). F2 PASS-7 NOT CLEAN (D-158): 1C/3H/4M/4L; novelty MODERATE; 2 axes CONVERGED (SPB formula / VP-INDEX). TRAJECTORY 23/24/17/13/13/13/12 (convergence approaching). Clean-pass counter 0/3. F3 BLOCKED — adversarial reconvergence (3 clean passes) required. Remediation round-7 pending.**
