...
phase_status: "F2-adversarial-convergence 0/3 clean — STRICT mode (human-elected 2026-06-12); 8 passes run, Pass 9 in progress"
...
arp_f2_adversary_convergence_counter: 0/3  # STRICT mode — zero findings any severity across all 4 slices required
arp_f2_convergence_trajectory: "15→20→~8→~15→~6→~4→~4→~7→(P9 in progress) — 0/3 clean STRICT mode; core semantics SETTLED. Detail: phase-f5-adversarial/arp-f2-convergence-trajectory.md"
...
**wirerust v0.6.0 RELEASED (DNP3 TCP analyzer, issue #8). Feature: ARP security analyzer + etherparse 0.16→0.20.1 migration (F1 PASSED 2026-06-12); release target v0.7.0. F2 spec evolution IN PROGRESS — adversarial convergence 0/3 clean STRICT mode (human-elected 2026-06-12; 8 passes run; Pass 9 in progress; core semantics SETTLED; trajectory 15→20→~8→~15→~6→~4→~4→~7).**
...
| Feature: ARP analyzer — F2 Spec Evolution | **IN PROGRESS** — adversarial convergence 0/3 STRICT mode (human-elected 2026-06-12) | SLICED 4-slice method; 8 passes run; Pass 9 in progress; trajectory 15→20→~8→~15→~6→~4→~4→~7; core semantics SETTLED; 283 BCs; artifact versions updated through Pass-8 (ADR-008 v1.6, arch-delta v1.8, PRD v1.13, BC-INDEX v1.13); trajectory: `phase-f5-adversarial/arp-f2-convergence-trajectory.md` |
