...
|-------|---------|-------|-----------|
| BC-2.05.010 | v1.5 | `unclassified_port_counts` Populated with (TransportProto, u16) Keys — TCP via Dispatcher None-Target, UDP via Decode-Loop | Primary: `TransportProto` enum, `unclassified_port_counts` field + dual-gate, `udp_unclassified_counts`, lower_port normalization |
...
**Traces to:** BC-2.05.010 v1.5 PC-4, Invariant 1; ADR-012 Decision 6
...
**Traces to:** BC-2.05.010 v1.5 PC-1 (dual-gate), Postcondition 4; BC-2.05.011 v1.1 Postconditions 1, 3; ADR-012 Decision 6 Clarification
...
(traces to BC-2.05.010 v1.5 PC-1, PC-4; BC-2.05.011 v1.1 PC-1, PC-3; ADR-012 Decision 6 Clarification)
...
**Traces to:** BC-2.05.010 v1.5 PC-1, Postconditions 1–4, Invariants 1–6; BC-2.05.011 v1.1 Postconditions 1, 3, 6; ADR-012 Decision 6 Clarification
...
(traces to BC-2.05.010 v1.5 PC-1, PC-2, Postconditions 1, 3–4; BC-2.05.011 v1.1 PC-1, PC-3;
...
**Traces to:** BC-2.05.010 v1.5 Postcondition 3, Invariant 2; BC-2.05.011 v1.1 Postcondition 5, Invariant 4
...
(traces to BC-2.05.010 v1.5 PC-3, Invariant 2; BC-2.05.011 v1.1 PC-5, Invariant 4)
...
**Traces to:** BC-2.05.010 v1.5 PC-2..3, Postconditions 2–3, Invariants 3, 7; BC-2.05.011 v1.1 PC-2, Postcondition 2; ADR-012 Decision 6, Decision 10
...
(traces to BC-2.05.010 v1.5 PC-2..3, PC-2, Postconditions 2–3, Invariants 3, 7;
...
**Traces to:** BC-2.05.011 v1.1 VP table; BC-2.05.010 v1.5 VP table; ADR-012 Decision 6
...
(traces to BC-2.05.011 v1.1 VP table; BC-2.05.010 v1.5 VP table; ADR-012 Decision 6 Clarification)

### AC-153-007: VP-043 proptest harnesses — UDP gap-key seam exactness and DNS exclusion
**Traces to:** BC-2.05.010 v1.5 VP table; BC-2.05.011 v1.1 VP table; ADR-012 Decision 10
...
1. **Write Red-Gate tests first (TDD Step 1 — all must FAIL before implementation)**
   ...
   `#[allow(non_snake_case)]` — CI enforces `-D warnings`; the uppercase `test_BC_…` names violate
   `non_snake_case`. (The existing file uses per-function allows; the new module block uses a
   module-level allow for consistency.)
   Contents:
...
8. **Test namespace isolation + `non_snake_case` allow (DF-TEST-NAMESPACE-001):** ALL test functions in `mod story_153 { ... }` wrapper. `tests/dispatcher_tests.rs` carries `#[allow(non_snake_case)]` on each individual test function (confirmed: lines 858, 933, 1030, 1100, 1254, etc.), which is the existing file convention. The NEW `mod story_153 { }` block MUST ALSO carry `#[allow(non_snake_case)]` at module scope so all uppercase `test_BC_…` names within it are covered without requiring per-function attributes on every new test — CI enforces `-D warnings`.
...
| v1.9 | 2026-07-04 | BC-version reference synced (BC-2.05.010 v1.4→v1.5); no behavioral change. | F5-RECONCILE-COMPLETION |
