...
github_issue: 8
# BC status: BC-2.15.016 authored at v1.1 as of 2026-06-10
inputs:
...
When a frame with DIR=1 (master-direction, `control & 0x10 != 0`) is observed, `src` is appended to `flow.master_addrs_seen` if not already present. `flow.master_addrs_seen.len()` NEVER exceeds `MAX_MASTER_ADDRS = 64`. Once full, new source addresses are silently ignored.
...
5. **Implement master-addr tracking** — `is_master_frame(control: u8) -> bool` helper; cap-guarded push.
...
4. **Pending-request eviction is by minimum `request_ts`** — oldest entry evicted. The evicted entry generates NO T1691.001 timeout event. This is the DoS-safe overflow behavior.
5. **Forbidden dependencies**: `src/analyzer/dnp3.rs` MUST NOT depend on `src/analyzer/modbus.rs`.
