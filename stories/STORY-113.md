...
github_issue: 9
# BC status: 8 BCs (7 original + BC-2.16.016 v1.0 added 2026-06-23). Primary owner of BC-2.16.010 and BC-2.16.016 (summarize keys and unbounded-findings invariant introduced here). BC-2.16.010 now at v1.9 (silent-limit audit: 11→13 keys; added bindings_evicted and storm_counters_evicted).
...
### AC-013 (traces to BC-2.16.010 postcondition 1 — all thirteen summary keys present)
`ArpAnalyzer::summarize()` returns an `AnalysisSummary` containing exactly these thirteen keys
(exact string names): `"frames_analyzed"`, `"request_count"`, `"reply_count"`, `"other_opcode_count"`, `"bindings_tracked"`, `"spoof_findings"`, `"garp_findings"`, `"storm_findings"`, `"mismatch_findings"`, `"malformed_findings"`, `"malformed_frames"`, `"bindings_evicted"`, `"storm_counters_evicted"`. All values are `u64` (or compatible numeric). All keys present with value 0 when no frames processed (BC-2.16.010 EC-001).
- **Test:** `test_BC_2_16_010_summarize_zero_frames_all_thirteen_keys_zero`, `test_BC_2_16_010_summarize_key_names_exact`
...
| EC-012 | Zero frames processed | All thirteen summarize keys = 0 |

## Tasks

1. **Replace `ArpAnalyzer` stub** with full implementation: add `bindings: HashMap<[u8;4], BindingEntry>`, `storm_counters: HashMap<[u8;6], StormCounter>` (stub: empty map; D3 logic in STORY-115), and all counter fields (`frames_analyzed: u64`, `request_count: u64`, `reply_count: u64`, `other_opcode_count: u64`, `spoof_findings: u64` [remains 0 here], `garp_findings: u64`, `storm_findings: u64` [remains 0], `mismatch_findings: u64`, `malformed_findings: u64`, `malformed_frames: u64`, `bindings_evicted: u64` [incremented on each binding-table LRU eviction; remains 0 until eviction fires], `storm_counters_evicted: u64` [stub: remains 0 here; D3 eviction wired in STORY-115]).
...
9. **Implement `summarize(&self)`**: return `AnalysisSummary` with all thirteen canonical keys in their exact string names (including `"bindings_evicted"` and `"storm_counters_evicted"`; both 0 here until eviction fires or D3 is wired in STORY-115).
...
| AC-013 | `test_BC_2_16_010_summarize_zero_frames_all_thirteen_keys_zero`, `test_BC_2_16_010_summarize_key_names_exact` | Unit |
...
5. **Thirteen keys exactly** — the exact string key names from BC-2.16.010 v1.9 postcondition 1 are the contract, including `"bindings_evicted"` and `"storm_counters_evicted"`. Any deviation (e.g., `"garp_count"` instead of `"garp_findings"`) fails the summary contract test.
