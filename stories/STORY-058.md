...
`parse_errors` and `truncated_records` are ALWAYS incremented together for oversized records — never independently. Buffer clearing is unconditional: ALL buffered bytes for that direction are dropped (including any valid partial records that preceded the oversized one).
- **Test:** Unit test with valid ClientHello then oversized record; assert `handshakes_seen=1`, `parse_errors=1`, `truncated_records=1`
...
- **Test:** Unit test for exact boundary: 18,432 does not increment; 18,433 does
...
- **Test:** Unit test: append 65,537 bytes to empty `client_buf`; assert `client_buf.len() == 65,536`
...
- **Test:** Unit test: append 1 byte when buffer is full; assert buffer length unchanged at 65,536
...
- **Test:** Unit test: fill buffer to 65,536; append 1000 more bytes; assert `parse_errors == 0`, `truncated_records == 0`
...
- **Test:** Unit test sequence: valid ClientHello, then malformed handshake record; assert `handshakes_seen=1`, `parse_errors=1`, `truncated_records=0`
...
- **Test:** `test_summarize_output` (assert `detail` key ordering and `top_snis.len() <= 20`)
...
- **Test:** Unit test: fresh analyzer; call summarize; assert `detail["truncated_records"]` exists and equals 0
...
- **Test:** Unit test: send ApplicationData (0x17) record then valid ClientHello; assert `parse_errors=0`, `handshakes_seen=1`
...
- **Test:** `test_stop_after_handshake` (exercises this code path with app-data records after hellos)
...
- **Test:** Unit test: process ClientHello on flow A; call `on_flow_close` for flow A; assert `flows.len() == 0` and `sni_counts` still has the entry from flow A
...
- **Test:** Unit test: call `on_flow_close` for a key not in `flows`; assert no panic and state unchanged
...
| tests/tls_analyzer_tests.rs | modify | `test_oversized_sni_exceeds_record_payload_limit`, buffer cap tests, `test_parse_error_counter`, `test_summarize_output`, flow lifecycle tests |
