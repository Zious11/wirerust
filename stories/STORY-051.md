...
- **Test:** `test_ja3_grease_filtering`
...
- **Test:** `compute_ja3_is_grease_invariant` (proptest)
...
- **Test:** `is_grease_u16_matches_all_canonical_grease_values`
...
- **Test:** `compute_ja3_has_five_fields_and_hex_hash` (proptest)
...
- **Test:** `compute_ja3_is_order_sensitive` (proptest)
...
2. [ ] Verify Red Gate: all 10 AC tests fail before implementation
...
6. [ ] Write proptest for `compute_ja3_is_grease_invariant` (inserting GREASE at any position does not change hash)
7. [ ] Write proptest for `compute_ja3_is_order_sensitive` (different cipher order = different hash)
8. [ ] Write proptest for `compute_ja3s_is_deterministic_and_hex` (32 lowercase hex chars, deterministic)
...
| File | Action | Purpose |
|------|--------|---------|
| src/analyzer/tls.rs | modify | `is_grease_u16` (line 50-52), `compute_ja3` (lines 94-151), `compute_ja3s` (lines 153-173) |
| tests/tls_analyzer_tests.rs | modify | Add/extend JA3 GREASE filtering tests, proptest suites for JA3 and JA3S |
