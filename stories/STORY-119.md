...
3. **Per-bucket collapse** (BC-2.11.033 Invariant 3, BC-2.11.025 Invariant 5, C-1 resolution):
   ...
   - `collapse_findings_pass` at `:340` becomes a thin adapter: collects `findings.iter().collect()` (the `&[Finding]` parameter) and delegates to `collapse_findings_pass_refs`. It is NOT called directly from the grouped path.
...
**Additional sweep target (F3 round-7 — stale semantic prose):** The following files contain stale prose describing the retired three-variant / three-arm model without using the enum-variant token. Grep for `"three-variant"`, `"three fields"`, `"All three FindingsRender"`, `"three.arm"` (and semantic synonyms) in the migrated files and update them to the two-orthogonal-axis / four-arm vocabulary confirmed at F4 GREEN:
- `tests/reporter_terminal_tests.rs:3949` — module-level comment `(a three-variant enum)` (stale; update to `(a struct-of-two-orthogonal-enums: Grouping × Collapse)`)
- `tests/reporter_terminal_tests.rs:3995` — doc-comment `The enum has exactly three variants` (stale; update to `The struct has exactly two fields: grouping: Grouping and collapse: Collapse`)
- `tests/reporter_terminal_tests.rs:4005` — body comment `All three variants are distinct` (stale; update to describe Grouping×Collapse combinations)
- `tests/reporter_terminal_tests.rs:4038` — test name `test_BC_2_11_028_struct_has_exactly_three_fields_post_refactor` + body comment `these three fields exist on TerminalReporter` (stale; the test was written pre-reshape when TerminalReporter had `render: FindingsRender` as one of three fields; the comment referencing "three-variant enum refactor" is stale vocabulary — update to the two-orthogonal-axis reshape)
- `tests/reporter_terminal_tests.rs:4061` — doc-comment `All three FindingsRender arms` (stale; now a four-arm dispatch after the reshape — update to `All four FindingsRender (Grouping × Collapse) dispatch arms`)

**EXEMPT from semantic-prose sweep (with rationale):**
- Changelog stanzas in STORY files and BC files that historically reference "three-variant enum" — these are frozen historical records of what was true at the time; they must not be rewritten.
- ADR text that describes the pre-STORY-119 state as historical context — these describe the migration path, not the current state.
...
3. `grep -n "three-variant\|three fields\|All three FindingsRender\|three.arm" tests/reporter_terminal_tests.rs` returns zero lines for the non-exempt targets listed above (or each remaining hit must correspond to an EXEMPT entry with rationale, e.g., a frozen changelog stanza).
...
- **v1.12 (F3 adversarial round-7 remediation, 2026-06-18):** Fix 1 (Pass A F-A-001 HIGH — compile-blocker `self.findings` phantom field): AC-027 and Task 4 item 3 both referenced `self.findings.iter().collect()` — but `struct TerminalReporter` has no `findings` field (only `use_color`, `show_hosts_breakdown`, `render`); `collapse_findings_pass(&self, findings: &'a [Finding])` takes `findings` as a parameter. Both occurrences corrected to `findings.iter().collect()` (the `&[Finding]` parameter); AC-027 also gains the explicit adapter body `let refs: Vec<&Finding> = findings.iter().collect(); self.collapse_findings_pass_refs(&refs)`. Full `self.` sweep performed: all other `self.` hits verified as valid (method calls or accesses to `self.render`, which is a real field). Fix 2 (Pass C F-C-001 MEDIUM — sprint-state blocked_by convention): STORY-119 `blocked_by: []` corrected to `blocked_by: [STORY-120]` — static dependency-edge set, consistent with every other done/pending story in the file; inline comment clarifies STORY-120 is delivered so live readiness is unblocked; NOTE block in sprint-state.yaml header gains one-line definition of blocked_by = static dep-edge set to prevent recurrence. Fix 3 (Pass A F-A-002 MEDIUM — Task 5 disambiguation note missing third sibling): Note updated to enumerate both pre-existing siblings — `:2072` (`test_BC_2_11_025_grouped_mode_bypasses_collapse`) and `:4140` (`test_BC_2_11_025_grouped_mode_bypasses_collapse_structurally`) — with their distinct verification roles; :4140 existence confirmed via grep. Fix 4 (Pass A F-A-003 MEDIUM — Task 7 sweep misses stale three-variant prose): Task 7 extended with explicit stale-prose targets at tests/reporter_terminal_tests.rs lines 3949, 3995, 4005, 4038, 4061; grep patterns added; third falsifiable gate added; EXEMPT list with rationale for frozen changelog stanzas and ADR historical-context text.
