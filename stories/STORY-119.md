...
The grouped-collapse header path applies the same verdict/confidence color-ladder as the color-selection block in `render_findings_collapsed` (locate by the `Likely` + `High` branch: `red().bold()` block; line shifts after STORY-122/A) to a pre-color string that ALREADY INCLUDES the ` (xN)` suffix. The ladder: `Likely+High` → `red().bold()`; `Likely+other` → `yellow`; `Possible` → `yellow`; `Inconclusive` → `cyan`; `Unlikely` → `dimmed`. Appending the suffix AFTER the ANSI color-reset sequence is NON-CONFORMANT.
...
**Scope:** The existing `collapse_findings_pass` function (locate by signature `fn collapse_findings_pass`; approx. `:340-360` pre-STORY-122/A; line range shifts after STORY-122/A grows the dispatch block).
...
3. **Per-bucket collapse** (BC-2.11.033 Invariant 3, BC-2.11.025 Invariant 5, C-1 resolution):
   ...
   - `collapse_findings_pass` (locate by signature `fn collapse_findings_pass`) is the thin adapter (Task 1); NOT called from the grouped path directly.
   - Reference: ADR-0003 "Collapse-API Shape" subsection; F2 design-note §5.2.1.
...
4. **Render each group** (BC-2.11.031, BC-2.11.032, BC-2.11.034):
   ...
   - **N≥2 (collapsed group):**
     ...
     b. Apply color-ladder (same as the color-selection block in `render_findings_collapsed` — locate by the `Likely` + `High` branch: `red().bold()` block; line shifts after STORY-122/A): `Likely+High` → `red().bold()`; `Likely+other` → `yellow`; `Possible` → `yellow`; `Inconclusive` → `cyan`; `Unlikely` → `dimmed`. Apply with `use_color` guard.
...
2. **Verdict rank is 4-valued, ascending:** Likely=0, Possible=1, Inconclusive=2, Unlikely=3 (source-confirmed in `verdict_rank` function in `terminal.rs`; locate by `fn verdict_rank`). Within-bucket sort is ascending by rank value (lowest value = highest severity = sorted first). All BC citations on sort direction use this vocabulary.
...
- **v2.3 (F3-resplit round-3 remediation, 2026-06-19):** Fix 3 (Pass A F-A-004 LOW, continued): AC-024 body text changed from brittle `collapse_findings_pass at :340` to content-based anchor `collapse_findings_pass (located by signature fn collapse_findings_pass; line shifts after STORY-122/A)`. Additional Task-body brittle-line audit: Task 1 Scope updated from `approx. :340-360` to content-based; Task 2 step 3 `collapse_findings_pass at :340` → content-based; AC-008 `terminal.rs:391` color-ladder reference → content-based (locate by `Likely` + `High` branch in `render_findings_collapsed`); Task 2 step 4b same `:391` → content-based; Previous Story Intelligence lesson 2 `terminal.rs:447-454` verdict_rank reference → `fn verdict_rank` content-based. Stable pre-dispatch anchors (`:73` COLLAPSE_EVIDENCE_SAMPLES; `src/main.rs:107`/`:108` run_analyze params) retained as-is — these are not affected by STORY-122/A's dispatch-block growth.
